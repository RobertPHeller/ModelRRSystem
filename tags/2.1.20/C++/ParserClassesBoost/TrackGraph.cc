...
TrackGraph::~TrackGraph()
{
	...
	for (tie(vi, vi_end) = vertices(nodes); vi != vi_end; vi++) {
		Node n = *vi;
		if (nodes[n].tgr != NULL) delete nodes[n].tgr;
		if (nodes[n].tpo != NULL) delete nodes[n].tpo;
	}
	nodes.clear();
...
ostream& operator << (ostream& stream,TrackGraph& graph)
{
	...
	for (nid = graph.LowestNode(); nid < graph.HighestNode(); nid++) {
		if (graph.FindNode(nid) == graph.none) continue;
...
float TrackGraph::EdgeLength(int nid, int edgenum)
{
	Node n = FindNode(nid);
	if (n == none) return -2.0;
	else {
...
#endif
		while (rn0 >= 0 &&
			c_nodes[newnode].FindSegmentIndex(rn0) == -1 &&
			NumEdges(rn0) == 2)
		{
#ifdef DEBUG
...
#endif
		if (rn0 >= 0 && c_nodes[newnode].FindSegmentIndex(rn0) != -1) {rn0 = -1;}
		while (rn1 >= 0 &&
			c_nodes[newnode].FindSegmentIndex(rn1) == -1 &&
			NumEdges(rn1) == 2)
		{
#ifdef DEBUG
...
#endif
		if (rn1 >= 0 && c_nodes[newnode].FindSegmentIndex(rn1) != -1) {rn1 = -1;}
...
void TrackGraph::computeHeads()
{
	...
	int num = strong_components(nodes, &component[0],
				    root_map(&root[0]).
				    color_map(&color[0]).
