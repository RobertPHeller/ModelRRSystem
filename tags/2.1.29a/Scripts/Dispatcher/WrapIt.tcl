...
#  puts stderr "*** WrapIt::CMriLibDir = $CMriLibDir"
  variable MRDLibDir [glob -nocomplain [file join $CodeLibrary MRD]]
#  puts stderr "*** WrapIt::MRDLibDir = $MRDLibDir"
...
#  puts stderr "*** WrapIt::CopyCommonLibFiles = $CopyCommonLibFiles"
  proc WrapIt {filename writeprogfun {needcmri no} {needmrd no}} {
    variable TclKit
    set compress 1
    set ropts -readonly
...
#      puts stderr "*** WrapIt::WrapIt: pkgLine = $pkgLine"
    ...
    if {$needmrd} {
      variable MRDLibDir
      file copy $MRDLibDir [file join $filename lib [file tail $MRDLibDir]]
    }
