...
catch {SplashWorkMessage {Loading Print Code} 88}
...
global LargePrinterImage
set LargePrinterImage {}
catch {
  global ImageDir
...
snit::type printConfigurationDialog {
  ...
  typeconstructor {
    ...
    $dialog add -name help -text Help -command [list BWHelp::HelpTopic PrintConfigurationDialog]
...
snit::type printDialog {
  ...
  typeconstructor {
    global LargePrinterImage
    set dialog [Dialog::create .printDialog -class PrintDialog \
    			-class PrintDialog -image $LargePrinterImage \
			-default 0 -cancel 2 -modal local -transient yes \
    ...
    $dialog add -name cancel -text Cancel -command [mytypemethod _Cancel]
    $dialog add -name help -text Help -command [list BWHelp::HelpTopic PrintDialog]
    set frame [$dialog getframe]
    set headerframe $frame.headerframe
    set iconimage $headerframe.iconimage
    set headerlabel $headerframe.headerlabel
  ...
  typemethod draw {args} {
    ...
    $printprogram configure -text "[TimeTableConfiguration getoption pdflatex]"
    return [Dialog::draw $dialog]    
  }
  typemethod _Cancel {} {
    Dialog::withdraw $dialog]
  ...
  typemethod _Print {} {
    ...
    TtInfoMessage draw -message "$latexfile Generated."
    set latexcmd "[$printprogram cget -text] [file rootname $latexfile]"
    if {$_Run3TimesFlag} {
      RunSubprocess "$latexcmd"
      RunSubprocess "$latexcmd"
      RunSubprocess "$latexcmd"
    } else {
      RunSubprocess "$latexcmd"
    }
    TtInfoMessage draw -message "Ran: $latexcmd"
    if {$_PostProcessCommandFlag} {
      RunSubprocess "[$postprocesscommandLE cget -text]"
      TtInfoMessage draw -message "Ran: [$postprocesscommandLE cget -text]"
    }
    return [Dialog::enddialog $dialog print]
  }
}

snit::widget subprocess {
  TtStdShell Subprocess
  ...
  method processDone {errorText} {
    if {![string equal "$errorText" {}]} {writeText "$errorText"}
    $dismisbutton configure -state normal
    incr _Running -1
  }
  method wait {} {
...
$::Main menu entryconfigure file Print... -command PrintTimeTable -state normal
image create photo PrintButtonImage -file [file join $CommonImageDir print.gif]
$::Main toolbar addbutton tools print -image PrintButtonImage \
				      -command PrintTimeTable \
				      -helptext "Print Timetable"
}
