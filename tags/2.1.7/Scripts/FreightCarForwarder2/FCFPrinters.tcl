...
snit::type OpenPrinterDialog {
  ...
  typeconstructor {
    set dialog [Dialog::create .openPrinterDialog \
		-class OpenPrinterDialog -bitmap questhead -default 0 \
		-cancel 1 -modal local -transient yes -parent . \
		-side bottom -title {Open Printer}]
    $dialog add -name ok -text OK -command [mytypemethod _Openit]
    $dialog add -name cancel -text Cancel -command [mytypemethod _Cancelit]
    $dialog add -name help -text Help \
		-command [list BWHelp::HelpTopic OpenPrinterDialog]
...
#    catch "$printerCommand" message
    if {[llength [info commands TheSystem]] == 0} {
      set title "Print lists for Unknown Railroad"
    } else {
      set title "Print lists for [TheSystem SystemName]"
    }
    $printerCommand Printer "$pfile" "$title"
...
proc ClosePrinter {} {
  if {[llength [info commands Printer]] > 0} {
    if {![Printer IsOpenP]} {
      tk_messageBox -icon warning -type ok -message "Printer is not open."
      rename Printer {}
    } else {
      Printer ClosePrinter
      rename Printer {}
      tk_messageBox -icon info -type ok -message "Printer is closed."
    }
  } else {
    tk_messageBox -icon warning -type ok -message "Printer is not open."
  }
...
proc PrintYardLists {} {
  ...
  if {![TheSystem RanAllTrains]} {
    tk_messageBox -icon error -type ok -message "Please Run the trains first"
    return
  }
