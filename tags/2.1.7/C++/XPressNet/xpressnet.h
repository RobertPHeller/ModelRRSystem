...
/** XPressNet C++ Serial Port Interface.
 ...
 GMBH (6/2003 third edition)\cite{Xpress03}. This code
  ...
  Basically, the way this code works is to use a class (described on
  \Ref{Class:XPressNet}) to interface to the serial port attached to one
  of Lenz's serial port adapters (LI100, LI100F, or LI101). 

 */
//@{

#ifdef SWIG
%typemap(tcl8,out) CommandStationResponse::TypeCode {
#if SWIGVERSIONMINOR == 1 && SWIGVERSIONMAJOR == 1
	char *typeCodeString = (char *) CommandStationResponse::TypeCodeString($source);
	Tcl_SetStringObj(tcl_result,typeCodeString,-1);
#else
...
/** \Label{Class:CommandStationResponse}
 Base response class.  All responses are derived from this class.
 */
...
public:
	...
	///  Default destructor (never called).
...
/** \Label{Class:NormalOperationResumed}
  Normal operation resumed.*/
...
/** \Label{Class:TrackPowerOff} Track power off. */
...
/// \Label{Class:EmergencyStop} Emergency stop.
...
/// \Label{Class:ServiceModeEntry} Service mode entry.
...
/** \Label{Class:ProgrammingInfoShortCircuit}
	Programming info. ``short-circuit''.*/
...
/** \Label{Class:ProgrammingInfoDataByteNotFound}
	Programming info. ``data byte not found''.*/
...
/** \Label{Class:ProgrammingInfoCommandStationBusy}
  Programming info. ``command station busy''.*/
...
/** \Label{Class:ProgrammingInfoCommandStationReady}
  Programming info. ``command station ready''. */
...
#ifdef SWIG
%typemap(tcl8,out) ServiceModeResponse::ServiceModeType {
#if SWIGVERSIONMINOR == 1 && SWIGVERSIONMAJOR == 1
	char *serviceModeTypeString = (char *) ServiceModeResponse::ServiceModeTypeString($source);
	Tcl_SetStringObj(tcl_result,serviceModeTypeString,-1);
#else
...
/// \Label{Class:ServiceModeResponse}  Service mode response.
...
#ifdef SWIG
%typemap(tcl8,out) SoftwareVersion::CommandStationTypeCode {
#if SWIGVERSIONMINOR == 1 && SWIGVERSIONMAJOR == 1
	char * commandStationTypeString = (char *) SoftwareVersion::CommandStationTypeCodeString($source);
	Tcl_SetStringObj(tcl_result,commandStationTypeString,-1);
#else
...
/// \Label{Class:SoftwareVersion}   Software version.
...
#ifdef SWIG
%typemap(tcl8,out) CommandStationStatus::StartModeType {
#if SWIGVERSIONMINOR == 1 && SWIGVERSIONMAJOR == 1
	char *startModeString = (char *) CommandStationStatus::StartModeTypeString($source);
	Tcl_SetStringObj(tcl_result,startModeString,-1);
#else
	Tcl_SetStringObj($result, (char *) CommandStationStatus::StartModeTypeString(( CommandStationStatus::StartModeType)$1),-1);
#endif
}

%typemap(tcl8,in) CommandStationStatus::StartModeType {
#if SWIGVERSIONMINOR == 1 && SWIGVERSIONMAJOR == 1
	int l;
	char *s = Tcl_GetStringFromObj($source,&l);
	if (strncasecmp(s,"manual",l) == 0) {
		$target = CommandStationStatus::Manual;
	} else if (strncasecmp(s,"automatic",l) == 0) {
		$target = CommandStationStatus::Automatic;
	} else {
...
/// \Label{Class:CommandStationStatus}  Command station status.
...
/// \Label{Class:TransferErrors}  Transfer errors.
...
/// \Label{Class:CommandStationBusy}   Command station busy.
...
/** \Label{Class:InstructionNotSupported}
	 Instruction not supported by command station.*/
...
#ifdef SWIG
%typemap(tcl8,out) AccessoryDecoderInformation::AccessoryTypeCode {
#if SWIGVERSIONMINOR == 1 && SWIGVERSIONMAJOR == 1
	char *accessoryTypeCodeString = (char *) AccessoryDecoderInformation::AccessoryTypeCodeString($source);
	Tcl_SetStringObj(tcl_result,accessoryTypeCodeString,-1);
#else
...
%typemap(tcl8,out) AccessoryDecoderInformation::TurnoutStatusCode {
#if SWIGVERSIONMINOR == 1 && SWIGVERSIONMAJOR == 1
	char *turnoutStatusCodeString = (char *) AccessoryDecoderInformation::TurnoutStatusCodeString($source);
	Tcl_SetStringObj(tcl_result,turnoutStatusCodeString,-1);
#else
...
%typemap(tcl8,out) AccessoryDecoderInformation::NibbleCode {
#if SWIGVERSIONMINOR == 1 && SWIGVERSIONMAJOR == 1
	char *nibbleCodeString = (char *) AccessoryDecoderInformation::NibbleCodeString($source);
	Tcl_SetStringObj(tcl_result,nibbleCodeString,-1);
#else
	Tcl_SetStringObj($result, (char *) AccessoryDecoderInformation::NibbleCodeString((AccessoryDecoderInformation::NibbleCode)$1),-1);
#endif
}

%typemap(tcl8,in) AccessoryDecoderInformation::NibbleCode {
#if SWIGVERSIONMINOR == 1 && SWIGVERSIONMAJOR == 1
	int l;
	char *s = Tcl_GetStringFromObj($source,&l);
	if (strncasecmp(s,"lower",l) == 0) {
		$target = AccessoryDecoderInformation::Lower;
	} else if (strncasecmp(s,"upper",l) == 0) {
		$target = AccessoryDecoderInformation::Upper;
	} else {
...
/// \Label{Class:AccessoryDecoderInformation}   Accessory decoder information.
...
#ifdef SWIG
%typemap(tcl8,out) LocomotiveInformation::SpeedStepModeCode {
#if SWIGVERSIONMINOR == 1 && SWIGVERSIONMAJOR == 1
	char *speedStepModeCodeString = (char *) LocomotiveInformation::SpeedStepModeCodeString((LocomotiveInformation::SpeedStepModeCode)$1);
	Tcl_SetStringObj(tcl_result,speedStepModeCodeString,-1);
#else
...
%typemap(tcl8,in)  LocomotiveInformation::SpeedStepModeCode {
#if SWIGVERSIONMINOR == 1 && SWIGVERSIONMAJOR == 1
	int l;
	char *s = Tcl_GetStringFromObj($source,&l);
	if (strncasecmp(s,"s14",l) == 0) {
		$target = LocomotiveInformation::S14;
	} else if (strncasecmp(s,"s27",l) == 0) {
		$target = LocomotiveInformation::S27;
	} else if (strncasecmp(s,"s28",l) == 0) {
		$target = LocomotiveInformation::S28;
	} else if (strncasecmp(s,"s128",l) == 0) {
		$target = LocomotiveInformation::S128;
	} else {
...
%typemap(tcl8,out) LocomotiveInformation::DirectionCode {
#if SWIGVERSIONMINOR == 1 && SWIGVERSIONMAJOR == 1
	char *directionCodeString = (char *) LocomotiveInformation::DirectionCodeString($source);
	Tcl_SetStringObj(tcl_result,directionCodeString,-1);
#else
	Tcl_SetStringObj($result, (char *) LocomotiveInformation::DirectionCodeString((LocomotiveInformation::DirectionCode)$1),-1);
#endif
}

%typemap(tcl8,in) LocomotiveInformation::DirectionCode {
#if SWIGVERSIONMINOR == 1 && SWIGVERSIONMAJOR == 1
	int l;
	char *s = Tcl_GetStringFromObj($source,&l);
	if (strncasecmp(s,"forward",l) == 0) {
		$target = LocomotiveInformation::Forward;
	} else if (strncasecmp(s,"reverse",l) == 0) {
		$target = LocomotiveInformation::Reverse;
	} else {
...
/// \Label{Class:LocomotiveInformation}   Locomotive information.
...
/// \Label{Class:FunctionStatus}   Function status.
...
#ifdef SWIG
%typemap(tcl8,out) LocomotiveAddress::AddressTypeCode {
#if SWIGVERSIONMINOR == 1 && SWIGVERSIONMAJOR == 1
	char *AddressTypeCodeString = (char *) LocomotiveAddress::AddressTypeCodeString($source);
	Tcl_SetStringObj(tcl_result,AddressTypeCodeString,-1);
#else
...
/// \Label{Class:LocomotiveAddress} Locomotive address.
...
#ifdef SWIG
%typemap(tcl8,out) DoubleHeaderInformation::SpeedStepModeCode {
#if SWIGVERSIONMINOR == 1 && SWIGVERSIONMAJOR == 1
	char *speedStepModeCodeString = (char *) DoubleHeaderInformation::SpeedStepModeCodeString($source);
	Tcl_SetStringObj(tcl_result,speedStepModeCodeString,-1);
#else
...
%typemap(tcl8,in)  DoubleHeaderInformation::SpeedStepModeCode {
#if SWIGVERSIONMINOR == 1 && SWIGVERSIONMAJOR == 1
	int l;
	char *s = Tcl_GetStringFromObj($source,&l);
	if (strncasecmp(s,"s14",l) == 0) {
		$target = DoubleHeaderInformation::S14;
	} else if (strncasecmp(s,"s27",l) == 0) {
		$target = DoubleHeaderInformation::S27;
	} else if (strncasecmp(s,"s28",l) == 0) {
		$target = DoubleHeaderInformation::S28;
	} else if (strncasecmp(s,"s128",l) == 0) {
		$target = DoubleHeaderInformation::S128;
	} else {
		Tcl_SetStringObj(tcl_result,"Unknown speed step mode code: '",-1);
		Tcl_AppendToObj(tcl_result, s, l);
		Tcl_AppendToObj(tcl_result,"', should be one of {s14 s27 s28 s128}",-1);
...
%typemap(tcl8,out) DoubleHeaderInformation::DirectionCode {
#if SWIGVERSIONMINOR == 1 && SWIGVERSIONMAJOR == 1
	char *directionCodeString = (char *) DoubleHeaderInformation::DirectionCodeString($source);
	Tcl_SetStringObj(tcl_result,directionCodeString,-1);
#else
...
%typemap(tcl8,in) DoubleHeaderInformation::DirectionCode {
#if SWIGVERSIONMINOR == 1 && SWIGVERSIONMAJOR == 1
	int l;
	char *s = Tcl_GetStringFromObj($source,&l);
	if (strncasecmp(s,"forward",l) == 0) {
		$target = DoubleHeaderInformation::Forward;
	} else if (strncasecmp(s,"reverse",l) == 0) {
		$target = DoubleHeaderInformation::Reverse;
	} else {
		Tcl_SetStringObj(tcl_result,"Unknown direction code: '",-1);
		Tcl_AppendToObj(tcl_result, s, l);
		Tcl_AppendToObj(tcl_result,"', should be one of {forward reverse}",-1);
...
/// \Label{Class:DoubleHeaderInformation}  Double header information.
...
#ifdef SWIG
%typemap(tcl8,out) DoubleHeaderMuError::ErrorTypeCode {
#if SWIGVERSIONMINOR == 1 && SWIGVERSIONMAJOR == 1
	char *errorCodeString = (char *) DoubleHeaderMuError::ErrorTypeCodeString($source);
	Tcl_SetStringObj(tcl_result,errorCodeString,-1);
#else
...
/// \Label{Class:DoubleHeaderMuError}   Double header or MU error.
...
#ifdef SWIG
%typemap(tcl8,out) LI100Message::MessageTypeCode {
#if SWIGVERSIONMINOR == 1 && SWIGVERSIONMAJOR == 1
	char *messageTypeCodeString = (char *) LI100Message::MessageTypeCodeString($source);
	Tcl_SetStringObj(tcl_result,messageTypeCodeString,-1);
#else
...
/// \Label{Class:LI100Message}  LI100 messages.
...
/**  \Label{Class:XPressNet}
  Main XPressNet interface class.  This class implements the interface logic
...
class XPressNet {
public:
	/**   \Label{Class:XPressNet:Constructor} The constructor opens
	...
	/**   \Label{Class:XPressNet:Destructor} The destructor restores
