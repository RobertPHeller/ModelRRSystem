...
/** The Division class implements a single division, which contains a number of
    contigious stations. A division has a name, a symbol, an area, a home yard,
    and a list of stations.
 */
class Division {
public:
#ifndef SWIG
/// Default constructor.  All fields are initialized to empty or NULL values.
	Division() {name = (char *)""; home = NULL; symbol = '\0'; area = '\0';}
/** Copy constructor.  A new division is created as a copy of an existing
  division.
 ...
 */
	Division(char s,Industry * h,char a, const char *n)
	{
		name = n;
		home = h;
		symbol = s;
...
/// Return the division's home yard.
	Industry * Home() const {return home;}
 ...
 */
	const Station *TheStation(int i) const {
		if (i < stations.size() && i >= 0) return stations[i];
		else return NULL;
	}
...
/// A map of divisions, by integer index.
typedef map<int, Division *, less<int> > DivisionMap;
/// A map of divisions, by division symbol.
