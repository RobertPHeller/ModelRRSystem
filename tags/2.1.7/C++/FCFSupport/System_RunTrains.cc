			        ...
			        PrinterDevice          *printer)
{
	...
	const Industry *ix;
	const Train *tx;
	int pageNum, lineNum, tmpTotalCars, listCon;
	string tempMessage;
	CarVector::const_iterator Cx;
	const Car *car;
	int Gx;
	...
	if (printAlpha) {
	  ...
	  for (copies = 0; copies < forend; copies++) {
	    Log->LogMessage(LogMessageCallback::Infomational,"Printing Yard Switchlist -- by Car\n");
		...
		lineNum = -1;
		tmpTotalCars = 0;
		tempMessage = "Cars List for ";
		tempMessage += ix->Name();
		tempMessage += "\n";
		Log->LogMessage(LogMessageCallback::Infomational,tempMessage);
...
//		it is picked up twice from the same location!
	...
	if (printList) {
	  ...
	  for (copies = 0; copies < forend; copies++) {
	    Log->LogMessage(LogMessageCallback::Infomational,"Printing Yard Pickups -- by Train\n");
	    for (Ix = industries.begin(); Ix != industries.end(); Ix++) {
	      if ((Ix->second) == NULL) continue;
	      ix = Ix->second;
	      if (ix->Type() == 'Y') {
	      	...
	      	pageNum = 1;
	      	int lineRem = -1;
	      	tempMessage  = "Check Train Pickups List for ";
	      	tempMessage += ix->Name();
	      	tempMessage += "\n";
	      	Log->LogMessage(LogMessageCallback::Infomational,tempMessage);
		for (Tx = trains.begin(); Tx != trains.end(); Tx++) {
		  ...
		  if (tmpTotalCars > 0) {
		    ...
		    tempMessage  = "Pickup Report for Train ";
		    tempMessage += tx->Name();
		    tempMessage += "\n";
		    Log->LogMessage(LogMessageCallback::Infomational,tempMessage);
		    printer->SetTypeSpacing(PrinterDevice::One);
...
//	           TmpTotalCars% > 0
	...
	if (printList) {
	  ...
	  if (printLtwice) forend = 2;
	  for (copies = 0; copies < forend; copies++) {
	    Log->LogMessage(LogMessageCallback::Infomational,"Printing Yard Drop Offs -- by Train\n");
	    for (Ix = industries.begin(); Ix != industries.end(); Ix++) {
	      if ((Ix->second) == NULL) continue;
	      ix = Ix->second;
	      if (ix->Type() == 'Y') {
		if (ix->divisionControlList.find('D') == string::npos) continue;
	      	...
	      	pageNum = 1;
	      	int lineRem = -1;
		tempMessage  = "Check Train Dropoffs List for ";
	      	tempMessage += ix->Name();
	      	tempMessage += "\n";
	      	Log->LogMessage(LogMessageCallback::Infomational,tempMessage);
		for (Tx = trains.begin(); Tx != trains.end(); Tx++) {
		  ...
		  if (tmpTotalCars > 0) {
		    ...
		    tempMessage  = "Drop Report for Train ";
		    tempMessage += tx->Name();
		    tempMessage += "\n";
		    Log->LogMessage(LogMessageCallback::Infomational,tempMessage);
		    printer->SetTypeSpacing(PrinterDevice::One);
...
//	          TmpTotalCars% > 0
	...
	if (printDispatch) {
	  PrintDispatcher("Manifests",'M',printer);
	  PrintDispatcher("Locals",'W',printer);
	}
			           ...
			           PrinterDevice          *printer) 
{
	...
	if (trainPrintOK) {
	  ...
	  GetCarStatus(car,status,carTypeDescr);
	  printer->Put(" DROP "); printer->Put(car->Marks());
	  printer->Tab(19);
	  printer->Put(car->Number());
	  printer->Tab(28);
	  printer->Put(car->Length()); printer->Put("ft");
	...
	*Lx = NULL;
	...
	message += car->Number();
	message += " is ";
	message += TheCarType(car->Type())->Type();
	message += " dest = ";
	message += car->Destination()->Name();
	message += "\n";
...
// is going.
	...
	if (trainPrintOK) {
...
#endif
	  printer->Put(" PICKUP "); printer->Put(car->Marks());
	  printer->Tab(19);
	  printer->Put(car->Number());
	  printer->Tab(28);
	  printer->Put(car->Length()); printer->Put("ft");
	  ...
	  if (wayFreight) {
	    ...
	    printer->Tab(110);
	    printer->Put("for "); printer->PutLine(car->Destination()->MyStation()->Name());
	  } else {
	    printer->Put("to "); printer->Put(car->Location()->Name());
	    printer->Tab(110);
	    printer->Put("dest "); printer->PutLine(car->Destination()->MyStation()->Name());
	  }
	}
#ifdef DEBUG
  ...
  for (Cx = cars.begin(); Cx != cars.end(); Cx++) {
    ...
    if (Total == 0) {
      ...
      sprintf(buffer,"%-21s%-7s%-8s%-8s%-8s%-8s%s\n%s\n","Cars Moved","Type",
		"Prv","1st","2nd","3rd","Destination",string(79,'-').c_str());
	Log->LogMessage(LogMessageCallback::Infomational,buffer);
    }
    GetCarStatus(car,status,carTypeDescr);
    Dest = car->Destination();
