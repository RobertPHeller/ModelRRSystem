...
%typemap(tcl8,out) Train::TrainType {
	...
	switch ($source) {
		case Train::Unknown:
			Tcl_SetStringObj(tcl_result,"Unknown",-1);
			break;
		case Train::Wayfreight:
			Tcl_SetStringObj(tcl_result,"Wayfreight",-1);
			break;
		case Train::BoxMove:
			Tcl_SetStringObj(tcl_result,"BoxMove",-1);
			break;
		case Train::Manifest:
			Tcl_SetStringObj(tcl_result,"Manifest",-1);
			break;
		case Train::Passenger:
			Tcl_SetStringObj(tcl_result,"Passenger",-1);
			break;
	}
#else
	switch ($1) {
...
/** Full constructor.  Initialize the class instance from a set of parameters.
  ...
  @param p A flag to indicate if a pickup / dropoff sheet should be printed
	   for this train.
...
#ifndef SWIG
/// A map of trains, indexed by integer.
typedef map<int, Train *, less<int> > TrainMap;
/// A map of trains, indexed by string.
