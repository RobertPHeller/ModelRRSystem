	...
	for (AssignLoop = 1; AssignLoop <= 2; AssignLoop++) {
...
#endif
	  sprintf(messageBuffer,"Car Assignment in Progress\nOuter Loop Initialization: %d",AssignLoop);
#ifdef DEBUG
	  ...
	  for (Cx = cars.begin(); Cx != cars.end(); Cx++) {
	    ...
	    if ((int)donefract != donepercent) {
	      ...
	      sprintf(messageBuffer,"%6.2f%% Done",donefract);
	  ...
	  if (Random() < 0.5) {
	    ...
	    ForStart = cars.begin();
	    ForStep = 1;
	    sprintf(messageBuffer,"Checking cars from 0 to %d\n",cars.size()-1);
	  } else {
	    ForEnd = cars.begin()-1;
	    ForStart = cars.end()-1;
	    ForStep = -1;
	    sprintf(messageBuffer,"Checking cars from %d to 0\n",cars.size()-1);
	  }
	  ...
	  for (Cx = ForStart;Cx != ForEnd;Cx += ForStep) {
	    ...
	    sprintf(messageBuffer,"Processing car %s %s\n",car->Marks(),car->Number());
	    log->LogMessage(LogMessageCallback::Infomational,messageBuffer);
	    sprintf(messageBuffer,"Cars inspected %d\n",CountCars);
	    log->LogMessage(LogMessageCallback::Infomational,messageBuffer);
	    sprintf(messageBuffer,"Cars Assigned  %d\n",RouteCars);
	    log->LogMessage(LogMessageCallback::Infomational,messageBuffer);
	    sprintf(messageBuffer,"Last Industry  %d\n\n\n",LastIx->first);
	    log->LogMessage(LogMessageCallback::Infomational,messageBuffer);
	    sprintf(messageBuffer,"%s %s %s at %s\n",
	    	    (car->tmpStatus?"Loaded":"Empty"),
	    ...
	    if ((Ix->second) != car->Location()) {
	      ...
	      sprintf(messageBuffer,"Assign %s %s %s is %s\n",
			car->Marks(),car->Number(),
			carTypes[car->Type()]->Type(),Status.c_str());
	      log->LogMessage(LogMessageCallback::Infomational,messageBuffer);
	      sprintf(messageBuffer," Now at %s\n",car->Location()->Name());
	      log->LogMessage(LogMessageCallback::Infomational,messageBuffer);
	      sprintf(messageBuffer," Send to %s\n",car->Destination()->Name());
	      log->LogMessage(LogMessageCallback::Infomational,messageBuffer);
//	      sprintf(messageBuffer," IndsAssignLen = %d IndsUsedLen = %d\n",
...
//	      log->LogMessage(LogMessageCallback::Infomational,messageBuffer);
	...
	bool hflag = true;
	WIP->ProgressStart("Car Assignment In Progress\nCars withour assignments");
	donefract = 0.0;
	donepercent = 0;
	WIP->ProgressUpdate(0,"  0.00% Done");
	for (Cx = cars.begin(); Cx != cars.end(); Cx++) {
