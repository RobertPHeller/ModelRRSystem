...
//    it travels out, turns, and returns thru the same area.
	...
	for (Gx = 1; Gx <= TotalStations; Gx++) {
		sprintf(messageBuffer,
			"Error reading %s -- short file (STATIONS)!",
		...
		if (vlist.size() != 4) {
			if (outmessage != NULL) {
				sprintf(messageBuffer,
					"Error reading %s STATIONS entry syntax error, expected 4 values, got \"%s\"!",
		...
		vword = trim(vlist[0]);
		sprintf(messageBuffer,"Error reading %s STATIONS entry syntax error, expected station number, got \"%s\"!",
					systemFile.FullPath().c_str(),vword.c_str());
		if (!StringToIntRange(vword,Sx,1,TotalStations,messageBuffer,outmessage)) return false;
		if (stations[Sx] != NULL) {
			if (outmessage != NULL) {
				sprintf(messageBuffer,
					"Error reading %s STATIONS entry syntax error, duplicated station number: %d at %s!",
		...
		vword = trim(vlist[2]);
		sprintf(messageBuffer,"Error reading %s STATIONS entry syntax error, expected Division number, got \"%s\"!",
					systemFile.FullPath().c_str(),vword.c_str());
		...
		if (val == 0) continue;
			...
			if (D == NULL) {
				if (outmessage != NULL) {
					sprintf(messageBuffer,
						"Bad Division number for Station #%s (%s): %d!",
		...
		stations[Sx] = new Station(vlist[1].c_str(),D,vlist[3].c_str());
