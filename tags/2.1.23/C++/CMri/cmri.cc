		     ...
		     CardType card,int dl,char **outmessage)
{
    ...
    if (ttyfd < 0) {
    	if (outmessage != NULL) {
    	    strcpy(messageBuffer,"Port not open!");
    	    *outmessage = new char[strlen(messageBuffer)+1];
	    strcpy(*outmessage,messageBuffer);
	}
	return;
    ...
    if (ua < 0 || ua > 127) {
	if (outmessage != NULL) {
	    sprintf(messageBuffer,"Ua out of range: %d",ua);
	    *outmessage = new char[strlen(messageBuffer)+1];
	    strcpy(*outmessage,messageBuffer);
	}
	return;
	...
	case SUSIC:
	    ...
	    for (i = 0; i < ns; i++) {
			...
			// No card, make sure this is last element.
	    	    	...
	    	    	case 3:	// Invalid card type OR hole
	    	    	    if (outmessage != NULL) {
	    	    	    	sprintf(messageBuffer,"Invalid CT(%d) = %d or CT positioning error",i,(*CT)[i]);
	    	    	    	*outmessage = new char[strlen(messageBuffer)+1];
				strcpy(*outmessage,messageBuffer);
			    }
			    return;
		...
		if (noct != no) {
		    if (outmessage != NULL) {
		    	sprintf(messageBuffer,"Number of output ports counted in CT (%d) not equal to no (%d)",noct,no);
		    	*outmessage = new char[strlen(messageBuffer)+1];
			strcpy(*outmessage,messageBuffer);
		    }
		    return;
		...
		if (nict != ni) {
		    if (outmessage != NULL) {
		    	sprintf(messageBuffer,"Number of input ports counted in CT (%d) not equal to no (%d)",nict,ni);
		    	*outmessage = new char[strlen(messageBuffer)+1];
			strcpy(*outmessage,messageBuffer);
		    }
		    return;
	...
	case SMINI:
	    ...
	    if (ni != 3) {
		if (outmessage != NULL) {
		    sprintf(messageBuffer,"ni must be = 3 for SMINI, got %d",ni);
		    *outmessage = new char[strlen(messageBuffer)+1];
		    strcpy(*outmessage,messageBuffer);
		}
		return;
	    ...
	    if (no != 6) {
		if (outmessage != NULL) {
		    sprintf(messageBuffer,"no must be = 6 for SMINI, got %d",no);
		    *outmessage = new char[strlen(messageBuffer)+1];
		    strcpy(*outmessage,messageBuffer);
		}
		return;
	    ...
	    if (ns < 0 || ns > 24) {
		if (outmessage != NULL) {
		    sprintf(messageBuffer,"ns out of range for 0 to 24 for SMINI, got %d",ns);
		    *outmessage = new char[strlen(messageBuffer)+1];
		    strcpy(*outmessage,messageBuffer);
		}
		return;
		...
		if (cti != 0) {
		    if (outmessage != NULL) {
			sprintf(messageBuffer,"Invalid CT(%d) = %d for SMINI",i,(*CT)[i]);
			*outmessage = new char[strlen(messageBuffer)+1];
			strcpy(*outmessage,messageBuffer);
		    }
		    return;
	    ...
	    if (ns != nscnt) {
		if (outmessage != NULL) {
		    strcpy(messageBuffer,"Signal count from CT is not equal to ns for SMINI");
		    *outmessage = new char[strlen(messageBuffer)+1];
		    strcpy(*outmessage,messageBuffer);
		}
		return;
    ...
    if (!transmit(ua,Init,ob,lm+1)) {
    	if (outmessage != NULL) {
    	    sprintf(messageBuffer,"Transmit error: %s",strerror(errno));
	    *outmessage = new char[strlen(messageBuffer)+1];
	    strcpy(*outmessage,messageBuffer);
	}
    }
...
List *CMri::Inputs(int ni,int ua,char **outmessage)
{
	...
	if (!readbyte(thebyte)) {
    	    if (outmessage != NULL) {
    		sprintf(messageBuffer,"Receive error: %s",strerror(errno));
		*outmessage = new char[strlen(messageBuffer)+1];
		strcpy(*outmessage,messageBuffer);
	    }
	    return NULL;
    ...
    if ((thebyte - 'A') != ua) {
    	if (outmessage != NULL) {
    	   sprintf(messageBuffer,"Received bad UA = %d",thebyte - 'A');
    	   *outmessage = new char[strlen(messageBuffer)+1];
	   strcpy(*outmessage,messageBuffer);
	}
	return NULL;
    ...
    if (thebyte != Read) {
    	if (outmessage != NULL) {
    	   sprintf(messageBuffer,"Received not = R for %d",ua);
    	   *outmessage = new char[strlen(messageBuffer)+1];
	   strcpy(*outmessage,messageBuffer);
	}
	return NULL;
	...
	if (thebyte == STX || thebyte == ETX) {
	    if (outmessage != NULL) {
	    	sprintf(messageBuffer,"No DLE ahead of STX or ETX for ua = %d",ua);
	    	*outmessage = new char[strlen(messageBuffer)+1];
		strcpy(*outmessage,messageBuffer);
	    }
	    delete result;
    ...
    if (thebyte != ETX) {
    	if (outmessage != NULL) {
    	   sprintf(messageBuffer,"ETX not properly received for ua %d",ua);
    	   *outmessage = new char[strlen(messageBuffer)+1];
	   strcpy(*outmessage,messageBuffer);
	}
	delete result;
