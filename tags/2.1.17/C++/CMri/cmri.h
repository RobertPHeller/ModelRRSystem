...
/** @memo A C++ mapping for a Tcl list.
  * @doc Re-sizable (manually only). Used to pass lists of integers (such as
...
/** @memo Main C/MRI interface class.
  * @doc This class implements the interface logic for
...
public:
	/** @memo Constructor. 
	  * @doc The constructor opens the serial port and initializes the port.
	...
	/** @memo Destructor.
	  * @doc The destructor restores the serial port's state and closes it.
	  */
	~CMri();
...
private:
	...
	/**  Maximum number of input I/O retries.
	  */
	int MaxTries;
	/** @memo  Data transmitter.
	  * @doc The data is built into a proper message and sent out the
