...
bool TimeTableSystem::CreateLaTeXTimetable(string filename_,char **outmessage)
{
	...
	if (NumberOfTrains() == 0) {
	  if (outmessage != NULL) {
	    sprintf(buffer,_("There are no Trains!"));
...
%";
	...
	if (!out) {
	  int err = errno;
	  if (outmessage != NULL) {
	    sprintf(buffer,_("TimeTableSystem::CreateLaTeXTimetable: Error creating file %s: %s"),
						 ...
						 char **outmessage)
{
	...
	for (igroup = 1; allTrains.size() > 0; igroup++) {
	  ...
	  if (!StringListFromString(trainlist,listOfTrains)) {
	    if (outmessage != NULL) {
	      sprintf(buffer,_("'Group,%d,Trains' print option has a syntax error ('%s')!"),
	  ...
	  if (listOfTrains.size() == 0 && allTrains.size() > 0) {
	    if (outmessage != NULL) {
	      sprintf(buffer,_("'Group,%d,Trains' print option is empty, but there are remaining trains!"),
	      	      igroup);
	      *outmessage = new char[strlen(buffer)+1];
	      strcpy(*outmessage,buffer);
	    }
