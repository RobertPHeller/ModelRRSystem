...
public:
	...
	static const char *TypeCodeString (TypeCode code) {
		switch (code) {
			...
			case LI100_VERSION:
				return "LI100_VERSION";
			case LI101_XPRESSNET_ADDRESS:
				return "LI101_XPRESSNET_ADDRESS";
			default: return "";
		}
	}
#endif
...
%typemap(in) CommandStationStatus::StartModeType {
	...
	if (strncasecmp(s,_("manual"),l) == 0) {
		$1 = CommandStationStatus::Manual;
	} else if (strncasecmp(s,_("automatic"),l) == 0) {
		$1 = CommandStationStatus::Automatic;
	} else {
		char *buffer, *fmt = _("Unknown start mode type: '%s', should be one of {manual automatic}");
		buffer = Tcl_Alloc(strlen(fmt)+l+5);
		sprintf(buffer,fmt,s);
		Tcl_SetStringObj(tcl_result,buffer,-1);
		Tcl_Free(buffer);
		return TCL_ERROR;
	}
}
...
%typemap(in) AccessoryDecoderInformation::NibbleCode {
	...
	char *s = Tcl_GetStringFromObj($input,&l);
	Tcl_Obj *tcl_result = Tcl_GetObjResult(interp);
	if (strncasecmp(s,_("lower"),l) == 0) {
		$1 = AccessoryDecoderInformation::Lower;
	} else if (strncasecmp(s,_("upper"),l) == 0) {
		$1 = AccessoryDecoderInformation::Upper;
	} else {
		char *buffer,*fmt=_("Unknown nibble code: '%s', should be one of {lower upper}");
		buffer = Tcl_Alloc(strlen(fmt)+l+5);
		sprintf(buffer,fmt,s);
		Tcl_SetStringObj(tcl_result,buffer,-1);
		Tcl_Free(buffer);
...
public:
	...
	static char const *AccessoryTypeCodeString (AccessoryTypeCode code) {
		switch (code) {
			case AccessoryWithoutFeedback: return _("AccessoryWithoutFeedback");
			case AccessoryWithFeedback:    return _("AccessoryWithFeedback");
			case FeedbackModule:           return _("FeedbackModule");
			case Reserved:                 return _("Reserved");
			default: return "";
		}
	}
#endif
	...
	static char const *TurnoutStatusCodeString (TurnoutStatusCode code) {
		switch (code) {
			case NotControlled: return _("NotControlled");
	...
	static char const *NibbleCodeString (NibbleCode code) {
		switch (code) {
			case Lower: return _("Lower");
			case Upper: return _("Upper");
			default: return "";
		}
	}
	/** @brief Constructor.
...
%typemap(in)  LocomotiveInformation::SpeedStepModeCode {
	...
	if (strncasecmp(s,"s14",l) == 0) {
		...
		char *buffer, *fmt = _("Unknown speed step mode code: '%s', should be one of {s14 s27 s28 s128}");
		buffer = Tcl_Alloc(strlen(fmt)+l+5);
		sprintf(buffer,fmt,s);
		Tcl_SetStringObj(tcl_result,buffer,-1);
		Tcl_Free(buffer);
...
%typemap(in) LocomotiveInformation::DirectionCode {
	...
	char *s = Tcl_GetStringFromObj($input,&l);
	Tcl_Obj *tcl_result = Tcl_GetObjResult(interp);
	if (strncasecmp(s,_("forward"),l) == 0) {
		$1 = LocomotiveInformation::Forward;
	} else if (strncasecmp(s,_("reverse"),l) == 0) {
		$1 = LocomotiveInformation::Reverse;
	} else {
		char *buffer, *fmt = _("Unknown direction code: '%s', should be one of {forward reverse}");
		buffer = Tcl_Alloc(strlen(fmt)+l+5);
		Tcl_SetStringObj(tcl_result,buffer,-1);
		Tcl_Free(buffer);
		return TCL_ERROR;
...
public:
	...
	static const char *DirectionCodeString(DirectionCode code) {
		switch (code) {
			case Forward: return _("Forward");
			case Reverse: return _("Reverse");
			default: return "";
		}
	}
	/** @brief Constructor.
...
public:
	...
	static const char *AddressTypeCodeString(AddressTypeCode code) {
		switch (code) {
			...
			case DoubleHeader: return _("DoubleHeader");
			case MultiUnitBase: return _("MultiUnitBase");
			case MultiUnit: return _("MultiUnit");
			case OtherOrNone: return _("OtherOrNone");
			default: return "";
		}
	}
#endif
...
public:
	...
	static const char *ErrorTypeCodeString (ErrorTypeCode code) {
		switch (code) {
			case NotOperatedOr0: return _("NotOperatedOr0");
			case UsedByAnotherDevice: return _("UsedByAnotherDevice");
			case UsedInANotherDHMU: return _("UsedInANotherDHMU");
			case SpeedNotZero: return _("SpeedNotZero");
			case NotMU: return _("NotMU");
			case NotMUBaseAddress: return _("NotMUBaseAddress");
			case CantDelete: return _("CantDelete");
			case StackFull: return _("StackFull");
			default: return "";
		}
	}
	/** @brief Constructor.
	...
	static const char *MessageTypeCodeString(MessageTypeCode code) {
		switch (code) {
			case ErrorBetweenLI100AndPC:
				return _("ErrorBetweenLI100AndPC");
			case ErrorBetweenLI100AndCommandStation:
				return _("ErrorBetweenLI100AndCommandStation");
			case UnknownCommunicationsError:
				return _("UnknownCOmmunicationsError");
			case Success:
				return _("Success");
			case NoTimeslot:
				return _("NoTimeslot");
			case BufferOverflow:
				return _("BufferOverflow");
			case Other:
				return _("Other");
			default: return "";
		}
...
/** LI100 Version Numbers.
  ...
  *
  */
class LI100VersionNumbers : public CommandStationResponse {
public:
#ifndef SWIG
	/** @brief Constructor.
	  * @param mbyte Message byte.
	  */
	LI100VersionNumbers(unsigned char hv, unsigned char sv) {
		Response_Type = LI100_VERSION;
		hardware_version = hv;
		software_version = sv;
	}
#endif
	/**  Return hardware version.
	  */
	unsigned char HardwareVersion() const {return hardware_version;}
	/**  Return software version.
	  */
	unsigned char SoftwareVersion() const {return software_version;}
	/** @brief Destructor.
	  */
	virtual ~LI100VersionNumbers() {}
#ifndef SWIG
private:
        /**  Hardware version.
	  */
...
/** LI101 XPress Net Address
  ...
  *
  */
class LI101XPressNetAddress : public CommandStationResponse {
public:
#ifndef SWIG
	/** @brief Constructor.
	  * @param mbyte Message byte.
	  */
	LI101XPressNetAddress(unsigned char addr) {
		Response_Type = LI101_XPRESSNET_ADDRESS;
		address = addr;
	}
#endif
	/**  Return XPressNet address.
	  */
	unsigned char Address() const {return address;}
	/** @brief Destructor.
	  */
	virtual ~LI101XPressNetAddress() {}
#ifndef SWIG
private:
        /**  Address.
	  */
...
public:
	...
	~XPressNet();
#ifdef SWIGTCL8
	%typemap(default) int timeout {$1 = 5;}
	CommandStationResponse::TypeCode CheckForResponse(int timeout,char **outmessage);
#else
	/**  Check for a response from the command station.
	  */
	CommandStationResponse::TypeCode CheckForResponse(time_t timeout=5,char **outmessage=NULL);
#endif
#ifndef SWIGTCL8
	/**  Return the next response from the command station.
	  */
	CommandStationResponse *GetNextCommandStationResponse(time_t timeout=5,char **outmessage=NULL);
#endif
	...
	/**  Fetch version numbers from LI100F and LI101.
	  */
	void GetLI100VersionNumbers();
	/** Set LI101's XPressNet address.
	  */
	void SetLI101Address(unsigned char addr);
#ifdef SWIGTCL8
	%extend {
		%apply int MyTcl_Result { int GetNextCommandStationResponse };
		int GetNextCommandStationResponse(Tcl_Interp *interp,int timeout) {
			char ** message = new char*;
			*message = NULL;
			CommandStationResponse *response = self->GetNextCommandStationResponse(timeout,message);
			if (*message != NULL) {
			...
			Tcl_Obj *responseObj;
			if (response == NULL) {
				responseObj = Tcl_NewStringObj("",0);
				Tcl_SetObjResult(interp,responseObj);
				return TCL_OK;
			}
			//fprintf(stderr,"*** [Tcl8] XPressNet::GetNextCommandStationResponse: response type is %d\n",response->ResponseType());
			switch (response->ResponseType()) {
				case CommandStationResponse::NO_RESPONSE_AVAILABLE:
					responseObj = Tcl_NewStringObj("",0);
					break;
				case CommandStationResponse::NORMAL_OPERATION_RESUMED:
					responseObj = NewSWIGNormalOperationResumed(interp,(void *)response);
					break;
				case CommandStationResponse::TRACK_POWER_OFF:
					responseObj = NewSWIGTrackPowerOff(interp,(void *)response);
					break;
				case CommandStationResponse::EMERGENCY_STOP:
					responseObj = NewSWIGEmergencyStop(interp,(void *)response);
					break;
				case CommandStationResponse::SERVICE_MODE_ENTRY:
					responseObj = NewSWIGServiceModeEntry(interp,(void *)response);
					break;
				case CommandStationResponse::PROGRAMMING_INFO_SHORT_CIRCUIT:
					responseObj = NewSWIGProgrammingInfoShortCircuit(interp,(void *)response);
					break;
				case CommandStationResponse::PROGRAMMING_INFO_DATA_BYTE_NOT_FOUND:
					responseObj = NewSWIGProgrammingInfoDataByteNotFound(interp,(void *)response);
					break;
				case CommandStationResponse::PROGRAMMING_INFO_COMMAND_STATION_BUSY:
					responseObj = NewSWIGProgrammingInfoCommandStationBusy(interp,(void *)response);
					break;
				case CommandStationResponse::PROGRAMMING_INFO_COMMAND_STATION_READY:
					responseObj = NewSWIGProgrammingInfoCommandStationReady(interp,(void *)response);
					break;
				case CommandStationResponse::SERVICE_MODE_RESPONSE:
					responseObj = NewSWIGServiceModeResponse(interp,(void *)response);
					break;
				case CommandStationResponse::SOFTWARE_VERSION:
					responseObj = NewSWIGSoftwareVersion(interp,(void *)response);
					break;
				case CommandStationResponse::COMMAND_STATION_STATUS:
					responseObj = NewSWIGCommandStationStatus(interp,(void *)response);
					break;
				case CommandStationResponse::TRANSFER_ERRORS:
					responseObj = NewSWIGTransferErrors(interp,(void *)response);
					break;
				case CommandStationResponse::COMMAND_STATION_BUSY:
					responseObj = NewSWIGCommandStationBusy(interp,(void *)response);
					break;
				case CommandStationResponse::INSTRUCTION_NOT_SUPPORTED:
					responseObj = NewSWIGInstructionNotSupported(interp,(void *)response);
					break;
				case CommandStationResponse::ACCESSORY_DECODER_INFORMATION:
					responseObj = NewSWIGAccessoryDecoderInformation(interp,(void *)response);
					break;
				case CommandStationResponse::LOCOMOTIVE_INFORMATION:
					responseObj = NewSWIGLocomotiveInformation(interp,(void *)response);
					
					break;
				case CommandStationResponse::FUNCTION_STATUS:
					responseObj = NewSWIGFunctionStatus(interp,(void *)response);
					break;
				case CommandStationResponse::LOCOMOTIVE_ADDRESS:
					responseObj = NewSWIGLocomotiveAddress(interp,(void *)response);
					break;
				case CommandStationResponse::DOUBLE_HEADER_INFORMATION:
					responseObj = NewSWIGDoubleHeaderInformation(interp,(void *)response);
					break;
				case CommandStationResponse::DOUBLE_HEADER_MU_ERROR:
					responseObj = NewSWIGDoubleHeaderMuError(interp,(void *)response);
					break;
				case CommandStationResponse::LI100_MESSAGE:
					responseObj = NewSWIGLI100Message(interp,(void *)response);
					break;
				case CommandStationResponse::LI100_VERSION:
					responseObj = NewSWIGLI100VersionNumbers(interp,(void *)response);
					break;
				case CommandStationResponse::LI101_XPRESSNET_ADDRESS:
					responseObj = NewSWIGLI101XPressNetAddress(interp,(void *)response);
					break;
				default:
					responseObj = Tcl_NewStringObj("",0);
					break;
...
#ifndef SWIG
protected:
	void _init(const char *port,char **outmessage);
private:
