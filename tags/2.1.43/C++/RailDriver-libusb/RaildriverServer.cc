 ...
 */
char RaildriverServer::pidfilename[256];
...
#endif
	// Log out connection.
	sprintf(writebuffer,_("Connection from %d.%d.%d.%d:%d accepted\n"),
...
#endif
	// Log us closed.
	sprintf(writebuffer,_("Connection from %d.%d.%d.%d:%d closed\n"),
		((remotesockaddr.sin_addr.s_addr) >>  0) & 0x0ff,
		((remotesockaddr.sin_addr.s_addr) >>  8) & 0x0ff,
		((remotesockaddr.sin_addr.s_addr) >> 16) & 0x0ff,
		((remotesockaddr.sin_addr.s_addr) >> 24) & 0x0ff,
 ...
 * 2) Open log file.
 * 3) Create listener socket.
 * 4) Connect to Rail Driver console.
 * 5) Establish signal handler.
 * 6) Store our PID
 */
bool RaildriverServer::Initialize(int argc, char *argv[])
{
...
#ifdef DEBUG
	logprintf("*** RaildriverServer::Initialize(): Created RaildriverIO instance, about to establish signal handler\n");
#endif
	...
	if (p == NULL) p = argv[0];
	busnum = 0; devnum = 0;
	sscanf(argv[1],"/proc/bus/usb/%03d/%03d",&busnum,&devnum);
	sprintf(pidfilename,"/var/run/%s-%03d-%03d.pid",p,busnum,devnum);
...
bool RaildriverServer::Shutdown()
{
	...
	sprintf(message,_("raildriverd stopping\n"));
	log(message);
	closelog();
...
void RaildriverServer::EventLoop()
{
	...
	while (true) {
		 ...
		 */  
#ifdef DEBUG
		logprintf("*** RaildriverServer::EventLoop(): Top of loop, about to check RailDriver->ReadInputs()\n");
#endif
		if (RailDriver->ReadInputs(mask,status)) {
			...
			cont |= true;
		} else if (status == LIBUSB_ERROR_NO_DEVICE) break; // Unplugged: break out!
		/*
