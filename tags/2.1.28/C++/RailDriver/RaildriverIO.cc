...
RaildriverIO::RaildriverIO(const char *device)
{
	...
	if (FindRailDriver(device)) {
		...
		// Die if open fails.
		if (rdHandle == NULL) {
			logprintf(_("RaildriverIO::RaildriverIO: usb_open failed: %s\n"),usb_strerror());
			exit(99);
		}
		...
		if (status < 0) {
			logprintf(_("RaildriverIO::RaildriverIO: usb_detach_kernel_driver_np failed: %s\n"),
				usb_strerror());
			/*exit(99);*/
		}
#endif
		...
		if (status < 0) {
			logprintf(_("usb_claim_interface failed: %s\n"),
					usb_strerror());
			exit(99);
		}
...
RaildriverIO::~RaildriverIO()
{
	...
	if (status < 0) {
		logprintf(_("RaildriverIO::~RaildriverIO: usb_release_interface failed: %s\n"),
				usb_strerror());
		exit(99);
	}
	status = usb_close(rdHandle);		// Close the device.
        if (status < 0) {
		logprintf(_("RaildriverIO::~RaildriverIO: usb_close failed: %s\n"),usb_strerror());
		exit(99);
        }
}
...
void RaildriverIO::SetLEDS(const char *ledstring)
{
	...
	if (status < 0) {
		logprintf(_("RaildriverIO::SetLEDS: usb_interrupt_write failed: %s\n"),usb_strerror());
	}
}
...
void RaildriverIO::SpeakerOn()
{
	...
	status = usb_interrupt_write(rdHandle,OUTPUTENDPOINT,(char *)buff,sizeof(buff),100);
	if (status < 0) {
		logprintf(_("RaildriverIO::SpeakerOn: usb_interrupt_write failed: %s\n"),usb_strerror());
	}
}
...
void RaildriverIO::SpeakerOff()
{
	...
	status = usb_interrupt_write(rdHandle,OUTPUTENDPOINT,(char *)buff,sizeof(buff),100);
	if (status < 0) {
		logprintf(_("RaildriverIO::SpeakerOff: usb_interrupt_write failed: %s\n"),usb_strerror());
	}
}
