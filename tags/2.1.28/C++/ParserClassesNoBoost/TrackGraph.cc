...
void TrackGraph::InsertBlock(int number, char * _name, char * _script, IntegerList *_tracklist)
{
	...
	newNode->numEdges = 0;
	newNode->name = _name;
	newNode->sensescript = _script;
	newNode->tracklist = _tracklist;
	newNode->nextNode = nodeTable[hash];
	newNode->length = -1.0;
	nodeTable[hash] = newNode;
}
...
void TrackGraph::InsertSwitchMotor(int number, int turnout, char * _name, char * _normal, char * _reverse, char * _pointsense)
{
	...
	Node *newNode = new Node;
	newNode->nodeId = number;
	newNode->nodeType = SwitchMotor;
	newNode->numEdges = 0;
	newNode->name = _name;
	newNode->turnoutnumber = turnout;
	newNode->sensescript = _pointsense;
	newNode->normalactionscript = _normal;
	newNode->reverseactionscript = _reverse;
	newNode->nextNode = nodeTable[hash];
	newNode->length = -1.0;
	nodeTable[hash] = newNode;
}
...
const char * TrackGraph::NormalActionScript(int nid) const
{
	Node *n = FindNode(nid);
	if (n == NULL) return NULL;
	else return n->normalactionscript;
}
...
const char * TrackGraph::ReverseActionScript(int nid) const
{
	Node *n = FindNode(nid);
	if (n == NULL) return NULL;
	else return n->reverseactionscript;
}
