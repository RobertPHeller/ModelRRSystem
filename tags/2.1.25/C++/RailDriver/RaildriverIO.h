...
public:
	...
	/** @brief   Constructor.
	  *
	  * The argument is the pathname under the
	  *   @c /proc/bus/usb/ directory that identifies the specific device.
	  *   Finds and opens the device and initializes various data objects,
	  *   generally preparing for I/O to the connnected rail driver
	  *   console.
	  *    @param device pathname under the @c /proc/bus/usb/ directory 
...
private:
	...
	/** @brief  Find the Rail Driver device.
	  *
	  *  Walk down the USB device tree finding
