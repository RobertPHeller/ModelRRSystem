...
bool System::ReadIndustries(char **outmessage)
{
	...
	if (!indusstream) {
	  if (outmessage != NULL) {
	    sprintf(messageBuffer,"Error opening industry file: %s",
...
//   IndsEmptyTypes  what CarTypes are accepted as empties
	for (Gx = 1; Gx <= TotalIndustries; Gx++) {
	  sprintf(messageBuffer,"Error reading %s -- short file (INDUSTRIES)!",
	  			industriesFile.FullPath().c_str());
...
//	    cerr << "*** System::ReadIndustries: vlist.size() = " << vlist.size() << endl;
	  ...
	  if (vlist.size() != 16) {
	    if (outmessage != NULL) {
	      sprintf(messageBuffer,"Error reading %s -- syntax error at %s file (INDUSTRIES)!",
...
//	  cerr << "*** System::ReadIndustries: vword = '" << vword << "'" << endl;
	  ...
	  if (industries[Ix] != NULL) {
	    if (outmessage != NULL) {
	      sprintf(messageBuffer,
			"Error reading %s INDUSTRIES entry syntax error, duplicated industry number: %d!",
	  ...
	  vword = trim(vlist[2]);
	  sprintf(messageBuffer,"Industry station number syntax in file (%s): %s",
		  industriesFile.FullPath().c_str(),vword.c_str());
	  ...
	  if (IndsStation == 0) continue;
	    ...
	    if (IStation == NULL) {
	      if (outmessage != NULL) {
	        sprintf(messageBuffer,"Industry station number out of range in file (%s): %s",
		  industriesFile.FullPath().c_str(),vword.c_str());
	        *outmessage = new char[strlen(messageBuffer)+1];
	        strcpy(*outmessage,messageBuffer);
	      }
	  ...
	  vword = trim(vlist[4]);
	  sprintf(messageBuffer,"Industry track length syntax error in file (%s): %s",
		  industriesFile.FullPath().c_str(),vword.c_str());
	  if (!StringToInt(vword,IndsTrackLen,messageBuffer,outmessage)) return false;vword = trim(vlist[2]);
	  vword = trim(vlist[5]);
	  sprintf(messageBuffer,"Industry assignable track length syntax error in file (%s): %s",
		  industriesFile.FullPath().c_str(),vword.c_str());
	  if (!StringToInt(vword,IndsAssignLen,messageBuffer,outmessage)) return false;vword = trim(vlist[2]);
	  vword = trim(vlist[6]);
	  sprintf(messageBuffer,"Industry Priority syntax error in file (%s): %s",
		  industriesFile.FullPath().c_str(),vword.c_str());
	  ...
	  if (vword[0] == 'Y' || vword[0] == 'y') IndsReload = true;
	    ...
	    if (outmessage != NULL) {
	      sprintf(messageBuffer,"Bad value for industry reload value: %s in %s",
	  ...
	  vword = trim(vlist[9]);
	  sprintf(messageBuffer,"Industry mirror number syntax in file (%s): %s at %s",
	  ...
	  vword = trim(vlist[10]);
	  sprintf(messageBuffer,"Industry plate number syntax in file (%s): %s",
		  industriesFile.FullPath().c_str(),vword.c_str());
	  if (!StringToInt(vword,IndsPlate,messageBuffer,outmessage)) return false;
	  vword = trim(vlist[11]);
	  sprintf(messageBuffer,"Industry weight class syntax in file (%s): %s",
		  industriesFile.FullPath().c_str(),vword.c_str());
	  if (!StringToInt(vword,IndsClass,messageBuffer,outmessage)) return false;
	  IndsDivList = trim(vlist[12]);
	  vword = trim(vlist[13]);
	  sprintf(messageBuffer,"Industry car length syntax in file (%s): %s",
		  industriesFile.FullPath().c_str(),vword.c_str());
...
//	  cerr << "*** System::ReadIndustries: industries[" << Ix << "] = " << industries[Ix] << endl;
	...
	for (map<int, int, less<int> >::const_iterator imirror = IndsMirrorMap.begin();
	  ...
	  if (industries[val] == NULL) {
	    if (outmessage != NULL) {
	      sprintf(messageBuffer,"Bad industry mirror number for industry #%d syntax in file (%s): %d",
