...
namespace eval HTMLHelp {
# HTMLHelp namespace, which contains the HTMLHelp snit widget adapter
# object and associated code.
# [index] HTMLHelp!namespace
  ...
  # Tab key bindings...
  bind HelpText <Tab> {
    set master [::HTMLHelp::HTMLHelp GetInstance %W]
    if {"$master" eq {}} {return}
    $master nextlink %W
    break
  }
  ...
  # A widget that implements a help dialog that renders HTML coded help
  # pages (generally generated from LaTeX using tex4ht's htlatex
  # script). This widget contains two panes, a narrow contents (a link
  # menu) pane on the left and a wider help text pane on the right.
  # Below these panes is a status label and a search text labeled entry,
  # with a set of four buttons along the bottom.  
  #
  # <option> -textwidth The initial width of the text pane.
  # <option> -width The overall width of the help dialog.
  # <option> -height The overall height of the help dialog.
  # <option> -side The side, top or bottom to put the sash. Default top.
  # <option> -helpdirectory The directory where the HTML files reside.
  # <option> -tableofcontents The file in the help directory that
  #		contains the table of contents (link menu).
  # [index] HTMLHelp!widget
...
#	puts stderr "*** $type GetInstance: $object does not exist"
    ...
    # A public typemethod to set the default values for the
    # -helpdirectory and -tableofcontents options.
    # <in> helpdir The default value for -helpdirectory.
    # <in> toc The default value for -tableofcontents.
    # [index] setDefaults!typemethod
    ...
    # <in> topic The help topic text to display help for.
    # [index] help!typemethod
	...
	error "Not a readable file, $options(-tableofcontents) (in $options(-helpdirectory)), for -tableofcontents!"
      ...
      installhull using Dialog -homogeneous 1 -separator 0 \
					 -modal none -parent . -place center \
					 -side bottom -title {Help} \
					 -transient 1 -anchor e \
					 -class HelpDialog
      set dframe [$hull getframe]
      install panes using PanedWindow $dframe.panes -side [from args -side]
      pack $panes -fill both -expand yes
      set left [$panes add -weight 1]
      install tocscroll using ScrolledWindow $left.tocscroll -relief sunken \
							     -borderwidth 2 \
							     -scrollbar both \
							     -auto both
      pack $tocscroll -fill both -expand yes
      ...
      bindtags $toc $bts
      set right [$panes add -weight 4]
      install textscroll using ScrolledWindow $right.textscroll -relief sunken \
							     -borderwidth 2 \
							     -scrollbar both \
							     -auto both
      ...
      install status  using Label $dframe.status -anchor w -relief flat \
						 -borderwidth 2 -justify left
      pack $status -fill x
      install command using LabelEntry $dframe.command
      pack $command -fill x
      $hull add -name close -text Close -underline 0 -command [mymethod _Close]
      $hull configure -cancel 0
      $hull add -name back -text Back -underline 0 -command [mymethod back]
      $hull add -name forward -text Forward -underline 0 -command [mymethod forward]
      $hull add -name help -text Help -underline 0 -command [mymethod helpTopic Help]
    ...
    # <in> topic The topic text to display help for.
    # [index] helpTopic!method
...
#      puts stderr "*** forwardcurrenttopic: topicstack = $topicstack, curtopicindex = $curtopicindex"
    ...
    method _Close {} {
      if {"[::grab current $win]" eq "$win"} {BWidget::grab release $win}
      $hull withdraw
    }
    method back {} {
    ...
    method searchforward {w} {
      $command configure -label "Search Forward:" -text "$lastsearch"
      $command bind <Return> [mymethod _SForward $w]
      focus $command
    }
    method _SForward {w} {
      set lastsearch [$command cget -text]
    ...
    method searchbackward {w} {
      $command configure -label "Search Backward:" -text "$lastsearch"
      $command bind <Return> [mymethod _SBackward $w]
      focus $command
    }
    method _SBackward {w} {
      set lastsearch [$command cget -text]
