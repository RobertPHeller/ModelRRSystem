...
proc OvalScale_Create {name} {
  ...
  set lfont [list "$data(-fontfamily)" -[expr int(ceil($width * .8))] bold]
  set data(font) "$lfont"
  switch -exact -- "$orientation" {
    vertical {
	...
	set vbox [list 0 0 0 0]
      ...
      array set baseRect \
	[list x1 $brx y1 $y x2 [expr $brx + $width] y2 [expr $y + $length]]
      array set initThumb \
	[list x1 $brx \
	      y1 [expr $y + (double($length) / 2.0) - (double($width) / 2.0)] \
	      x2 [expr $brx + $width] \
	...
	set vbox [list 0 0 0 0]
      ...
      array set baseRect \
	[list x1 $x y1 $bry x2 [expr $x + $length] y2 [expr $bry + $width]]
      array set initThumb \
	[list x1 [expr $x + (double($length) / 2.0) - (double($width) / 2.0)] \
	      y1 $bry \
	      x2 [expr $x + (double($length) / 2.0) + (double($width) / 2.0)] \
