...
XPressNet::XPressNet(const char *port, char **outmessage)
{
	...
	if (retval < 0) {
		if (outmessage != NULL) {
			sprintf(messageBuffer,_("Error from select on %s because %s."),port,strerror(errno));
			*outmessage = new char[strlen(messageBuffer)+1];
			strcpy(*outmessage,messageBuffer);
		}
		tcsetattr(ttyfd,TCSANOW,&savedtermios);
	...
	if (retval == 0) {	// No response, maybe a LI100 or LI100F at 9600bps
		...
		if (retval < 0) {
			if (outmessage != NULL) {
				sprintf(messageBuffer,_("Error from select on port %s because %s."),port,strerror(errno));
				*outmessage = new char[strlen(messageBuffer)+1];
				strcpy(*outmessage,messageBuffer);
			}
			tcsetattr(ttyfd,TCSANOW,&savedtermios);
		...
		if (retval == 0) {	// No response, nothing there??
			if (outmessage != NULL) {
				sprintf(messageBuffer,_("No response on port %s -- is a LI100/LI100F/LI101 connected?"),port);
				*outmessage = new char[strlen(messageBuffer)+1];
				strcpy(*outmessage,messageBuffer);
			}
			tcsetattr(ttyfd,TCSANOW,&savedtermios);
	...
	if (len != (message[0] & 0x0f)+1) {
		if (outmessage != NULL) {
			sprintf(messageBuffer,_("Read error on port %s: read %d bytes, header says %d bytes -- is a LI100/LI100F/LI101 connected?"),port,len,(message[0] & 0x0f)+1);
				*outmessage = new char[strlen(messageBuffer)+1];
				strcpy(*outmessage,messageBuffer);
		}
		tcsetattr(ttyfd,TCSANOW,&savedtermios);
	...
	if (xorbyte != message[(message[0] & 0x0f)+1]) {
		if (outmessage != NULL) {
			sprintf(messageBuffer,_("Bad X-Or-Byte on %s: computed 0x%02x, got 0x%02x -- is a LI100/LI100F/LI101 connected?"),port,xorbyte,message[(message[0] & 0x0f)+1]);
				*outmessage = new char[strlen(messageBuffer)+1];
				strcpy(*outmessage,messageBuffer);
		}
		tcsetattr(ttyfd,TCSANOW,&savedtermios);
	...
	if (message[0] != 0x02) {
		if (outmessage != NULL) {
			sprintf(messageBuffer,_("Bad response header byte (0x%02x) on %s -- is a LI100/LI100F/LI101 connected?"),message[0],port);
				*outmessage = new char[strlen(messageBuffer)+1];
				strcpy(*outmessage,messageBuffer);
		}
		tcsetattr(ttyfd,TCSANOW,&savedtermios);
...
CommandStationResponse::TypeCode XPressNet::CheckForResponse(char **outmessage)
{
	...
	if (len != (message[0] & 0x0f)+1) {
		if (outmessage != NULL) {
			sprintf(messageBuffer,_("Read error: read %d bytes, header says %d bytes -- is a LI100/LI100F/LI101 connected?"),len,(message[0] & 0x0f)+1);
				*outmessage = new char[strlen(messageBuffer)+1];
				strcpy(*outmessage,messageBuffer);
		}
		return CommandStationResponse::NO_RESPONSE_AVAILABLE;
