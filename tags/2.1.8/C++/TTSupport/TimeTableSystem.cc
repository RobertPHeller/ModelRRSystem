...
//#endif
	  ...
	  if (outmessage != NULL) {
	    if (layoverVector->size() != nstops) {
 	      strcpy(buffer,"Range Check: layover vector is the wrong size!");
	    } else if (cabnameVector->size() != nstops) {
 	      strcpy(buffer,"Range Check: cabname vector is the wrong size!");
	    } else if (storageTrackVector->size() != nstops) {
 	      strcpy(buffer,"Range Check: storage track vector is the wrong size!");
 	    }
	    *outmessage = new char[strlen(buffer)+1];
	    strcpy(*outmessage,buffer);
	  }
