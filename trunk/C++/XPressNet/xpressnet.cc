...
#define ADDRESS(address) (((address)<100)?(address):(address+0x0c000))
...
void XPressNet::_init(const char *port, char **outmessage)
{
	...
	if (ttyfd < 0) {
		if (outmessage != NULL) {
			sprintf(messageBuffer,_("Open of port %s failed because %s."),port,strerror(errno));
	...
	if (tcgetattr(ttyfd,&savedtermios) < 0) {
		if (outmessage != NULL) {
			sprintf(messageBuffer,_("Cannot get termios struct for %s because %s."),port,strerror(errno));
			*outmessage = new char[strlen(messageBuffer)+1];
			strcpy(*outmessage,messageBuffer);
		}
		close(ttyfd);
	...
	if (cfsetispeed(&currenttermios,B19200) < 0) {
		if (outmessage != NULL) {
			sprintf(messageBuffer,_("Cannot set termios struct in speed for %s because %s."),port,strerror(errno));
			*outmessage = new char[strlen(messageBuffer)+1];
			strcpy(*outmessage,messageBuffer);
		}
		close(ttyfd);
	...
	if (cfsetospeed(&currenttermios,B19200) < 0) {
		if (outmessage != NULL) {
			sprintf(messageBuffer,_("Cannot set termios struct out speed for %s because %s."),port,strerror(errno));
			*outmessage = new char[strlen(messageBuffer)+1];
			strcpy(*outmessage,messageBuffer);
		}
		close(ttyfd);
	...
	if (tcsetattr(ttyfd,TCSANOW,&currenttermios) < 0) {
		if (outmessage != NULL) {
			sprintf(messageBuffer,_("Cannot set termios struct for %s because %s."),port,strerror(errno));
			*outmessage = new char[strlen(messageBuffer)+1];
			strcpy(*outmessage,messageBuffer);
		}
		close(ttyfd);
	...
	// Send a BREAK.
#ifdef DEBUG
	fprintf(stderr,"*** XPressNet::XPressNet: Sending a BREAK @ 19200\n");
#endif
	if (tcsendbreak(ttyfd,0) < 0) {
		if (outmessage != NULL) {
			sprintf(messageBuffer,_("Cannot send BREAK %s because %s."),port,strerror(errno));
			*outmessage = new char[strlen(messageBuffer)+1];
			strcpy(*outmessage,messageBuffer);
		}
...
#ifdef DEBUG
	fprintf(stderr,"*** XPressNet::XPressNet: Sending startup message @ 19200\n");
#endif
	GetLI100VersionNumbers();
	while (1) {
	  CommandStationResponse *response = GetNextCommandStationResponse(5);
#ifdef DEBUG
	  fprintf(stderr,"*** XPressNet::XPressNet: response is %llx\n",(long long) response);
#endif
	  if (response == NULL) {
#ifdef DEBUG
		fprintf(stderr,"*** XPressNet::XPressNet: Setting BAUD rate to 9600\n");
#endif
	  	if (cfgetospeed(&currenttermios) == B9600) {
	  		if (outmessage != NULL) {
	  			sprintf(messageBuffer,_("No response on port %s -- is a LI100/LI100F/LI101F connected?"),port);
	  			*outmessage = new char[strlen(messageBuffer)+1];
				strcpy(*outmessage,messageBuffer);
			}
			tcsetattr(ttyfd,TCSANOW,&savedtermios);
		...
		// Set the input speed to 9600 (assuming a old LI100)
...
#ifdef DEBUG
		fprintf(stderr,"*** XPressNet::XPressNet: Sending startup message @ 9600\n");
#endif
		GetLI100VersionNumbers();
	  } else if (response->ResponseType() != CommandStationResponse::LI100_VERSION) {
#ifdef DEBUG
		fprintf(stderr,"*** XPressNet::XPressNet: Got response type: %s\n",CommandStationResponse::TypeCodeString(response->ResponseType()));
#endif
	  	delete response;
	  } else {
#ifdef DEBUG
...
CommandStationResponse::TypeCode XPressNet::CheckForResponse(time_t timeout,char **outmessage)
{
	// Error message buffer.
	static char messageBuffer[2048];
	// LI100 message buffer;
	unsigned char message[18], xorbyte;
	// Misc. integers.
	int i;
	// Variables for select/pselect
	fd_set rfds;
	struct timeval tv;
	int retval, len, count, l;
	...
	if (retval < 0) {
		if (outmessage != NULL) {
			sprintf(messageBuffer,_("Error from select because %s."),strerror(errno));
			*outmessage = new char[strlen(messageBuffer)+1];
			strcpy(*outmessage,messageBuffer);
		}
	...
	l = read(ttyfd,message,1);
	if (l < 1) {
		if (outmessage != NULL) {
			sprintf(messageBuffer,_("Read error: could not read header byte -- is a LI100/LI100F/LI101 connected?"));
			*outmessage = new char[strlen(messageBuffer)+1];
				strcpy(*outmessage,messageBuffer);
		}
		return CommandStationResponse::NO_RESPONSE_AVAILABLE;
	}
	len = read(ttyfd,message+1,(message[0] & 0x0f)+1)+1;
#ifdef DEBUG
	fprintf(stderr,"*** XPressNet::CheckForResponse: read back %d bytes\n",len);
	fprintf(stderr,"*** XPressNet::CheckForResponse: bytes: ");
	for (int i = 0; i < len; i++) {
	  fprintf(stderr,"%02X ",message[i]);
	}
	fprintf(stderr,"\n");
#endif
	if (len != (message[0] & 0x0f)+2) {
		if (outmessage != NULL) {
			sprintf(messageBuffer,_("Read error: read %d bytes, header says %d bytes -- is a LI100/LI100F/LI101 connected?"),len,(message[0] & 0x0f)+2);
				*outmessage = new char[strlen(messageBuffer)+1];
				strcpy(*outmessage,messageBuffer);
		}
		return CommandStationResponse::NO_RESPONSE_AVAILABLE;
...
#ifdef DEBUG
	fprintf(stderr,"*** XPressNet::CheckForResponse: xorbyte is %02X\n",xorbyte);
#endif
	if (xorbyte != message[(message[0] & 0x0f)+1]) {
		if (outmessage != NULL) {
			sprintf(messageBuffer,_("Bad X-Or-Byte: computed 0x%02x, got 0x%02x -- is a LI100/LI100F/LI101 connected?"),xorbyte,message[(message[0] & 0x0f)+1]);
				*outmessage = new char[strlen(messageBuffer)+1];
				strcpy(*outmessage,messageBuffer);
		}
		return CommandStationResponse::NO_RESPONSE_AVAILABLE;
	...
	switch (message[0] & 0x0f0) {
		case 0x00:
			if (message[0] == 0x01) {
				...
				if (message[1] == 0x01) {
					response = new LI101XPressNetAddress(message[2]);
				} else {
					response = new LI100VersionNumbers((((message[1] & 0x0f0) >> 4) * 10) + (message[1] & 0x0f),(((message[2] & 0x0f0) >> 4) * 10) + (message[2] & 0x0f));
				}
			} else {
				response = NULL;
			}
		...
		case 0xe0:
			switch (message[0] & 0x0f) {
				...
				case 0x06: {
					...
					LocomotiveInformation::DirectionCode dir;
					LocomotiveInformation::SpeedStepModeCode ssm = LocomotiveInformation::S14;
					unsigned char s = 0;
	...
	if (response == NULL) {
		if (outmessage != NULL) {
			sprintf(messageBuffer,_("Bad or illformed message received, ignored."));
			*outmessage = new char[strlen(messageBuffer)+1];
				strcpy(*outmessage,messageBuffer);
		}
		return CommandStationResponse::NO_RESPONSE_AVAILABLE;
...
CommandStationResponse * XPressNet::GetNextCommandStationResponse(time_t timeout,char **outmessage)
{
	if (ttyfd < 0) return NULL;

	if (responseList == NULL &&
	    CheckForResponse(timeout,outmessage) == CommandStationResponse::NO_RESPONSE_AVAILABLE)
		return NULL;
...
void XPressNet::LocomotiveInformationRequest(unsigned short int address)
{
	...
	message[0] = 0xe3;
	message[1] = 0x00;
	message[2] = (ADDRESS(address) >> 8) & 0x0ff;
					       ...
					       unsigned char speed)
{
	...
	fprintf(stderr,"*** XPressNet::SetLocomotiveSpeedAndDirection: message = ");
	for (i = 0; i < 6; i++) fprintf(stderr,"%02x ",message[i]);
	fprintf(stderr,"\n");
...
void XPressNet::AddressInquiryNextMUMember(unsigned char mtr,unsigned short int address)
{
	...
	message[1] = 0x01;
	message[2] = mtr;
	message[3] = (ADDRESS(address) >> 8) & 0x0ff;
...
/************************************************************************
 *									*
 * Get LI100 Version number.						*
 *									*
 ************************************************************************/

void XPressNet::GetLI100VersionNumbers()
{
...
/************************************************************************
 *									*
 * Set LI101 XPressNet Address.						*
 *									*
 ************************************************************************/

void XPressNet::SetLI101Address(unsigned char addr)
{
	unsigned char message[4];
	int i;
	if (ttyfd < 0) return;
