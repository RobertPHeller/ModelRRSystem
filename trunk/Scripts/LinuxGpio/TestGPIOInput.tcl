...
snit::widgetadaptor InputList {
    ...
    method addgpio {pinno} {
        if {[catch {linuxgpio::GpioInputActiveHigh %AUTO% -pinnumber $pinno} gpio]} {
...
snit::type TestGPIOInput {
    pragma -hastypeinfo    no
    pragma -hastypedestroy no
    pragma -hasinstances   no
    typecomponent main
    typecomponent   inputlist
    typeconstructor {
        set main [mainwindow .main]
        pack $main -expand yes -fill both
        set inputlist [InputList \
                        [$main scrollwindow getframe].ilist]
        $main scrollwindow setwidget $inputlist
        ...
        for {set off 0} {$off < $GPIOCount} {incr off} {
            $inputlist addgpio [expr {$Start + $off}]
        }
        $main showit
    }
    typemethod _exit {} {
        $inputlist destroyall
        ::exit
    }
}
