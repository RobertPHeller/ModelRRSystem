...
#      puts stderr "*** $type constructor: \[info script\] = [info script]"
    ...
    typemethod createnewDialog {} {
      ...
      $newDialog add -name help -text Help -command {BWHelp::HelpTopic NewCTCPanelWindowDialog}
    ...
    typemethod createselectPanelDialog {} {
      ...
      $selectPanelDialog add -name help -text Help \
			     -command {BWHelp::HelpTopic SelectPanelDialog}
...
#      puts stderr "*** $type create $self $args"
      ...
      $hull add -name help -text Help -command {BWHelp::HelpTopic AddPanelObjectDialog}
      ...
      set row 0
      foreach {rb0 rb1 rb2 rb3 rb4 rb5} {sWPlateRB sIGPlateRB codeButtonRB toggleRB lampRB cTCLabelRB straightBlockRB curvedBlockRB hiddenBlockRB stubYardRB throughYardRB crossingRB switchRB scissorCrossoverRB singleSlipRB doubleSlipRB threeWaySWRB schLabelRB} {
	foreach rb [list $rb0 $rb1 $rb2 $rb3 $rb4 $rb5] col {0 1 2 3 4 5} {
	  regsub {RB$} "$rb" {} name
...
Adjust x, y, and radius values and click Redraw"
      ...
      if {[lsearch -exact {SWPlate SIGPlate CodeButton Toggle Lamp CTCLabel} $objectType] < 0} {
...
#      puts stderr "*** $self packAndConfigureOptions: objtype = $objtype, options(-object) = $options(-object), opts are [$options(-ctcpanel) itemconfigure $options(-object)]"
	...
	switch -exact $opt {
	  ...
	  flipped {
		pack $flippedLCB -fill x
		$hascenterLCB configure -text [$options(-ctcpanel) itemcget $options(-object) -flipped]
	  }
	  position {
		pack $positionLCB -fill x
  ...
  snit::widgetadaptor SelectPanelObjectDialog {
    ...
    constructor {args} {
      ...
      $hull add -name cancel -text Cancel -command [mymethod _Cancel]
      wm protocol [winfo toplevel $win] WM_DELETE_WINDOW [mymethod _Cancel]
      $hull add -name help -text Help -command {BWHelp::HelpTopic SelectPanelObjectDialog}
      set frame [$hull getframe]
...
#      puts stderr "*** $type create $self $args"
      ...
      $hull add -name cancel -text Cancel -command [mymethod _Cancel]
      wm protocol [winfo toplevel $win] WM_DELETE_WINDOW [mymethod _Cancel]
      $hull add -name help -text Help -command {BWHelp::HelpTopic ConfigurePanelDialog}
      set frame [$hull getframe]
      install nameLE using LabelEntry $frame.nameLE -label "Name:" \
						    -labelwidth 15 \
						    -editable no
  ...
  snit::widgetadaptor AddCMRINodeDialog {
    ...
    constructor {args} {
      ...
      $hull add -name cancel -text Cancel -command [mymethod _Cancel]
      wm protocol [winfo toplevel $win] WM_DELETE_WINDOW [mymethod _Cancel]
      $hull add -name help -text Help -command {BWHelp::HelpTopic AddCMRINodeDialog}
      set frame [$hull getframe]
      install nameLE using LabelEntry $frame.nameLE -label "Name:" \
						    -labelwidth 18 \
						    -text {}
...
#      puts stderr "*** $self draw $args"
  ...
  snit::widgetadaptor SelectCMRINodeDialog {
    ...
    constructor {args} {
      ...
      $hull add -name cancel -text Cancel -command [mymethod _Cancel]
      wm protocol [winfo toplevel $win] WM_DELETE_WINDOW [mymethod _Cancel]
      $hull add -name help -text Help -command {BWHelp::HelpTopic SelectCMRINodeDialog}
      set frame [$hull getframe]
      install namePatternLE using LabelEntry $frame.namePatternLE \
					-label "Pattern:" \
					-labelwidth 18 \
