  ...
  snit::widget CTCPanelWindow {
	...
	-extramenus [list \
      ...
      $main menu insert file [_m "Menu|File|Open..."] \
		command -label [_m "Menu|File|Load..."] \
      ...
      $main menu entryconfigure file [_m "Menu|File|Save"] \
				...
				-dynamichelp [_ "Save window code"] \
				-accelerator {Ctrl-S}
      $main menu entryconfigure file [_m "Menu|File|Save As..."] \
				-command [mymethod saveas] \
				-dynamichelp [_ "Save window code"] \
				-accelerator {Ctrl-A}
      $main menu insert file [_m "Menu|File|Print..."] command \
	-label [_m "Menu|File|Wrap As..."] \
	-dynamichelp [_ "Wrap window code"] \
	-command [mymethod wrapas] \
	-accelerator {Ctrl-W} \
	-state disabled
      if {[WrapIt::CanWrapP]} {
	$main menu entryconfigure file [_m "Menu|File|Wrap As..."] -state normal
      }
      $main menu entryconfigure file [_m "Menu|File|Print..."] \
				-command [mymethod print] \
				-dynamichelp [_ "Print Panel"] \
				-accelerator {Ctrl-P}
      $main menu insert file [_m "Menu|File|Close"] command \
	-label [_m "Menu|File|Export as images..."] \
	-dynamichelp [_ "Export panel as images"] \
	-command [mymethod export] \
	-accelerator {Ctrl-E}
      $main menu entryconfigure file [_m "Menu|File|Close"] -command [mymethod close]
      ...
      $main menu add edit command \
		-label [_m "Menu|Edit|(Re-)Generate Main Loop"] \
		-command [mymethod GenerateMainLoop]
      $main menu add edit command \
		-label [_m "Menu|Edit|User Code"] \
		-command [mymethod EditUserCode]
      set modMenu [menu [$main mainframe getmenu edit].modules -tearoff no]
      $main menu add edit cascade \
		-label [_m "Menu|Edit|Modules"] \
		-menu $modMenu
      $modMenu  add command -label [_m "Menu|Edit|Modules|Track Work type"] \
		-command [mymethod AddModule TrackWork]
      $modMenu  add command -label [_m "Menu|Edit|Modules|Switch Plate type"] \
		-command [mymethod AddModule SwitchPlates]
      set sigMenu [menu $modMenu.sigMenu -tearoff no]
      $modMenu  add cascade -label [_m "Menu|Edit|Signals"] \
		-menu $sigMenu
	$sigMenu add command -label [_m "Menu|Edit|Signals|Two Aspect Color Light"] \
			     -command [mymethod AddModule Signals2ACL]
	$sigMenu add command -label [_m "Menu|Edit|Signals|Three Aspect Color Light"] \
			     -command [mymethod AddModule Signals3ACL]
	$sigMenu add command -label [_m "Menu|Edit|Signals|Three Aspect Search Light"] \
			     -command [mymethod AddModule Signals3ASL]
      $modMenu  add command -label [_m "Menu|Edit|Signals|Signal Plate type"] \
		-command [mymethod AddModule SignalPlates]
      $modMenu  add command -label [_m "Menu|Edit|Signals|Control Point type"] \
		-command [mymethod AddModule ControlPoints]
      $modMenu  add command -label [_m "Menu|Edit|Signals|Radio Group Type"] \
...
#				     -filetypes $filetypes \
	...
	file rename -force "$filename" "${filename}~"
      ...
      WrapIt::WrapIt $filename [mymethod writeprog] $options(-hascmri)
    }
    ...
    typemethod createPrintDialog {} {
      ...
      set papersizeLCB [LabelComboBox $frame.papersizeLCB -label [_mx "Label|Paper size:"] \
							  -labelwidth $lwidth \
							  -editable no \
    ...
    method print {} {
      ...
      set pdfobj [$type drawPrintDialog -parent $win \
					-filename $printfile]
      if {"$pdfobj" eq ""} {return}
      $pdfobj startPage
...
#      puts stderr "*** CTCPanelWindow::theimagetype: result = '$result'"
    ...
    typemethod createExportDialog {} {
      ...
      $_exportdialog add -name cancel	-text [_ "Button|Cancel"]  \
      ...
      set exportSchematicfileFE [FileEntry $frame.exportSchematicfileFE \
					-label [_mx "Label|Schematic Output file:"] \
					-labelwidth $lwidth \
      ...
      pack $exportSchematicfileCB -fill x
      set exportControlsfileFE [FileEntry $frame.exportControlsfileFE \
					-label [_mx "Label|Controls Output file:"] \
					-labelwidth $lwidth \
					-filetypes $imagefiletypes \
					-filedialog save]
      ...
      pack $exportControlsfileCB -fill x
    }
    typemethod _Export {} {
      set isOK yes
	...
	tk_messageBox -icon warning \
		      -message [_ "Not a support image type for Schematic file: %s" [theimagetype [$exportSchematicfileFE cget -text]]] \
	...
	tk_messageBox -icon warning \
		      -message [_ "Not a support image type for Controls file: %s" [theimagetype [$exportControlsfileFE cget -text]]] \
		      -parent $_exportdialog -type ok
	set isOK no
      }
      if {$isOK} {
...
#	puts stderr "*** $self GenerateMainLoop: start = $start, end = $end"
	...
	if {[catch {open "$tempName" w} fp]} {
	  tk_messageBox -type ok -icon error [_ "Could not create tempfile: %s" $fp]
	  return
	} else {
...
#	  puts stderr "*** $self draw: options(-object) = '$options(-object)'"
	...
	if {[string first {Range error: } "$error"] >= 0} {
	  ...
	  tk_messageBox -type ok -icon warning -parent $win \
			-message [_ "Range error: radius too small for points.\n\
...
#      puts stderr "*** $self packAndConfigureOptions: objtype = $objtype, options(-object) = $options(-object), opts are [$options(-ctcpanel) itemconfigure $options(-object)]"
  ...
  snit::widgetadaptor SelectPanelObjectDialog {
	...
	tk_messageBox -type ok -icon warning -message [_ "No such object: %s" [$selectedNameLE cget -text]]
	return
      }
...
#      puts stderr "*** $self draw $args"
	...
	edit {
	  ...
	  $hull itemconfigure add -text [_ "Button|Update"]
	  $hull configure -title [_ "Edit CMR/I node"]
	}
	add -
	default {
	  $nameLE configure -editable yes
	  $hull itemconfigure add -text [_ "Button|Add"]
  ...
  snit::widgetadaptor SelectCMRINodeDialog {
    ...
    method _Select {} {
      ...
      if {[lsearch -exact [$parent cmrinodelist] \
			  "[$selectedNameLE cget -text]"] < 0} {
	tk_messageBox -type ok -icon warning -message [_ "No such board: %s" [$selectedNameLE cget -text]]
	return
      }
      return [$hull enddialog "[$selectedNameLE cget -text]"]
    }
