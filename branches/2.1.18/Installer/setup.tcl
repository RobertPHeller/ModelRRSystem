...
#      puts stderr "*** AdditionalArchives::CheckAndPopulate: Including Windows-only archive $a for platform $::tcl_platform(platform)"
    ...
    pack [LabelEntry $archiveTFframe.dest \
...
proc FindArchivesAndComputeSizes_UNIX {} {
  ...
  set ::LittleDocFiles {}
  foreach f {COPYING README Readme.Centos} {
    set f1 [file join $::CDDir $f]
    if {[file exists "$f1"]} {lappend ::LittleDocFiles "$f1"}
  }
  if {[catch {
...
proc MainWindow {} {
  ...
  pack [Label $header.headtext -bg #242d2b -fg #70cfe0 -font {Times -32 bold} \
  ...
  foreach p {Startup InstallArchives DestDisk Installing AdditionalArchives 
		Installing2 Done} {
	...
	pack [Entry [$docsLF getframe].size -editable no -width 6\
		...
		-side left
	pack [Button $page.next -text "Next ==>" -command {set ::State InstallArchives}\
				-state disabled] \
	...
	namespace eval DestDisk {
		...
		switch -exact -- $::tcl_platform(platform) {
		  ...
		  windows {set destdir "C:/mrrsystem-$VERSION"}
	...
	pack [LabelEntry $pframe.spaceNeededLE -label "Space Needed:" \
		...
		-expand yes -fill x
	pack [LabelEntry $pframe.destdirLE -label "Destination Folder:" \
						-labelwidth 20 \
	...
	namespace eval AdditionalArchives {
		...
		array set archiveList [list \
			MRRSystem-2.1.17.tar.gz {{MRR System source code} tar-xzvf UNIX-only} \
			MRRSystem-2.1.17.zip {{MRR System source code} unzip Windows-only} \
