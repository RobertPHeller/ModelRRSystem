...
namespace eval xpressnet {
  ...
  snit::type CommandStationResponse {
	...
	INSTRUCTION_NOT_SUPPORTED {}
	SERVICE_MODE_RESPONSE {
	  eval [list install actualresponse using 
			xpressnet::ServiceModeResponse %%AUTO%%] $args
	}
	SOFTWARE_VERSION {
	  eval [list install actualresponse using xpressnet::SoftwareVersion \
		%%AUTO%%] $args
	}
	COMMAND_STATION_STATUS {
	  eval [list install actualresponse using \
		xpressnet::CommandStationStatus %%AUTO%%] $args
	}
	ACCESSORY_DECODER_INFORMATION {
	  eval [list install actualresponse using \
		xpressnet::AccessoryDecoderInformation %%AUTO%%] $args
	}
	LOCOMOTIVE_INFORMATION {
	  eval [list install actualresponse using \
		xpressnet::LocomotiveInformation %%AUTO%%] $args
	}
	FUNCTION_STATUS {
	  eval [list install actualresponse using xpressnet::FunctionStatus \
		%%AUTO%%] $args
	}
	LOCOMOTIVE_ADDRESS {
	  eval [list install actualresponse using xpressnet::LocomotiveAddress \
		%%AUTO%%] $args
	}
	DOUBLE_HEADER_INFORMATION {
	  eval [list install actualresponse using \
		xpressnet::DoubleHeaderInformation %%AUTO%%] $args
	}
	DOUBLE_HEADER_MU_ERROR {
	  eval [list install actualresponse using \
		xpressnet::DoubleHeaderMuError %%AUTO%%] $args
	}
	LI100_MESSAGE {
	  eval [list install actualresponse using xpressnet::LI100Message \
		%%AUTO%%] $args
	}
	LI100_VERSION {
	  eval [list install actualresponse using \
		xpressnet::LI100VersionNumbers %%AUTO%%] $args
	}
	LI101_XPRESSNET_ADDRESS {
	  eval [list install actualresponse using \
		xpressnet::LI101XPressNetAddress %%AUTO%%] $args
	}
      }
    }
  }
...
#
	...
	if {$response eq {}} {
	  if {[fconfigure $ttyfd -mode] eq "9600,n,8,1"} {
	    close $ttyfd
	    catch {unset ttyfd}
	    error [_ "No response on port %s -- is a LI100/LI100F/LI101F connected?" $port]
	    return
	  }
	  if {[catch {fconfigure $ttyfd -mode 9600,n,8,1} err]} {
	    close $ttyfd
	    catch {unset ttyfd}
	...
	return NO_RESPONSE_AVAILABLE
	  ...
	  [$self CheckForResponse $timeout] == NO_RESPONSE_AVAILABLE} {
	return {}
      }
	...
	# @param address Address of locomotive.
      xpressnet::DecoderLongAddress validate $address
      set message [list 0xe3 0x00 [expr {($address >> 8) & 0x0ff}] \
				  [expr {$address & 0x0ff}]]
      $self _appendXORByte message
      $self _transmit $message
    }
	...
	# @param address Address of locomotive.
      xpressnet::DecoderLongAddress validate $address
      set message [list 0xe3 0x07 [expr {($address >> 8) & 0x0ff}] \
				  [expr {$address & 0x0ff}]]
      $self _appendXORByte message
      $self _transmit $message
    }
	...
	# @param speed Desired speed.
      ...
      set addhbyte [expr {($address >> 8) & 0x0ff}]
	...
	# @param f4 Function 4.
      ...
      set message [list 0xe4 0x20 [expr {($address >> 8) & 0x0ff}] \
			[expr {$address & 0x0ff}]]
      if {$f0} {
	set fbyte 0x10
      } else {
	...
	# @param f8 Function 8.
      ...
      set message [list 0xe4 0x21 [expr {($address >> 8) & 0x0ff}] \
			[expr {$address & 0x0ff}]]
      set fbyte 0x00
	...
	# @param f12 Function 12.
      ...
      set message [list 0xe4 0x22 [expr {($address >> 8) & 0x0ff}] \
			[expr {$address & 0x0ff}]]
      set fbyte 0x00
	...
	# @param f4 Function 4.
      ...
      snit::boolean validate $f3
      snit::boolean validate $f4
      set message [list 0xe4 0x24 [expr {($address >> 8) & 0x0ff}] \
			[expr {$address & 0x0ff}]]
      if {$f0} {
	set fbyte 0x10
      } else {
	...
	# @param f8 Function 8.
      ...
      snit::boolean validate $f7
      snit::boolean validate $f8
      set message [list 0xe4 0x25 [expr {($address >> 8) & 0x0ff}] \
			[expr {$address & 0x0ff}]]
      set fbyte 0x00
      if {$f5} {set fbyte [expr {$fbyte | 0x01}]}
      if {$f6} {set fbyte [expr {$fbyte | 0x02}]}
	...
	# @param f12 Function 12.
      ...
      snit::boolean validate $f11
      snit::boolean validate $f12
      set message [list 0xe4 0x26 [expr {($address >> 8) & 0x0ff}] \
			[expr {$address & 0x0ff}]]
      set fbyte 0x00
      if {$f9} {set fbyte [expr {$fbyte | 0x01}]}
      if {$f10} {set fbyte [expr {$fbyte | 0x02}]}
	...
	# @param data Data to set.
      ...
      set message [list 0xe6 0x30 [expr {($address >> 8) & 0x0ff}] \
			[expr {$address & 0x0ff}] \
			[expr {0xec | (($cv >> 8) & 0x03)}] \
			[expr {$cv & 0x0ff}] \
			[expr {$data & 0x0ff}]]
	...
	lappend message 0x41
      ...
      lappend message [expr {($address >> 8) & 0x0ff}] \
			[expr {$address & 0x0ff}] \
			[expr {$mtr & 0x07f}]]
      $self _appendXORByte message
      $self _transmit $message
	...
	# @param mtr Multi-Unit address.
      xpressnet::DecoderLongAddress validate $address
      xpressnet::ConsistAddress validate $mtr
      set message [list 0xe4 0x42 [expr {($address >> 8) & 0x0ff}] \
			[expr {$address & 0x0ff}] \
			[expr {$mtr & 0x0ff}]]
      $self _appendXORByte message
      $self _transmit $message
	...
	# @param address Locomotive address.
      xpressnet::DecoderLongAddress validate $address
      set message [list 0xe3 0x05 [expr {($address >> 8) & 0x0ff}] \
			[expr {$address & 0x0ff}]]
      $self _appendXORByte message
      $self _transmit $message
    }
	...
	# @param address Locomotive address.
      xpressnet::DecoderLongAddress validate $address
      set message [list 0xe3 0x06 [expr {($address >> 8) & 0x0ff}] \
			[expr {$address & 0x0ff}]]
      $self _appendXORByte message
      $self _transmit $message
    }
	...
	# @param address Locomotive address.
      xpressnet::DecoderLongAddress validate $address
      set message [list 0xe3 0x44 [expr {($address >> 8) & 0x0ff}] \
			[expr {$address & 0x0ff}]]
      $self _appendXORByte message
      $self _transmit $message
    }
...
#
	...
	## @private The event handler.
      set last_response_type [$xpressnet CheckForResponse 0]
      if {$last_response_type ne "NO_RESPONSE_AVAILABLE"} {
	set response [$xpressnet GetNextCommandStationResponse 0]
