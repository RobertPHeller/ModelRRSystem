...
XpressNetEvent::XpressNetEvent(Tcl_Interp *interp,const char *eventScript,const char *port,char **outmessage)
{
	...
	last_response_type = CommandStationResponse::NO_RESPONSE_AVAILABLE;
...
/************************************************************************
 *									*
 * Event setup method.  Nothing here.					*
 *									*
 ************************************************************************/
...
void XpressNetEvent::eventCheck(int flags)
{
	...
	int theEventFlag = (flags & TCL_FILE_EVENTS);
#ifdef DEBUG
	fprintf(stderr,"***  XpressNetEvent::eventCheck: theEventFlag = %0X\n",theEventFlag);
#endif
	if (!(flags & TCL_FILE_EVENTS)) {
		return;
	}
#ifdef DEBUG
	fprintf(stderr,"***  XpressNetEvent::eventCheck(%X)\n",flags);
#endif

	outmessage = NULL;
	last_response_type = CheckForResponse(0,&outmessage);
	if (outmessage != NULL) {
		int mlen = strlen(outmessage);
#ifdef DEBUG
		fprintf(stderr,"*** XpressNetEvent::eventCheck: outmessage is %s\n",outmessage);
#endif
		delete outmessage;
	}
#ifdef DEBUG
	fprintf(stderr,"***  XpressNetEvent::eventCheck: rtype is %d\n",rtype);
#endif
	if (last_response_type != CommandStationResponse::NO_RESPONSE_AVAILABLE) {
#ifdef DEBUG
		fprintf(stderr,"***  XpressNetEvent::eventCheck: last_response_type is %d\n",last_response_type);
#endif
		struct event *evPtr =
...
{
#ifdef DEBUG
	fprintf(stderr,"***  XpressNetEvent::eventProc\n");
#endif
	if (!(flags & TCL_FILE_EVENTS)) {
		return 0;
	}
	if (last_response_type != CommandStationResponse::NO_RESPONSE_AVAILABLE) {
		CommandStationResponse *response = GetNextCommandStationResponse(0);
		if (response == NULL) return 1;
#ifdef DEBUG
		fprintf(stderr,"***  XpressNetEvent::eventProc: response is %lx\n",(long long)response);
#endif
		...
		switch (response->ResponseType()) {
			...
			case CommandStationResponse::LI100_VERSION:
				responseObj = NewSWIGLI100VersionNumbers(the_interp,(void *)response);
				break;
			case CommandStationResponse::LI101_XPRESSNET_ADDRESS:
				responseObj = NewSWIGLI101XPressNetAddress(the_interp,(void *)response);
				break;
			default:
				return 1;
		}			
