...
bool System::ReadOwners(char **outmessage)
{
	...
	if (!ownersstream) {
	  if (outmessage != NULL) {
	    sprintf(messageBuffer,"Error opening owners file: %s",
	...
	sprintf(messageBuffer,"Error reading owners file: %s",
		ownersFile.FullPath().c_str());
	if (!SkipCommentsGets(ownersstream,line,messageBuffer,outmessage)) return false;
	sprintf(messageBuffer,"TotalOwners number syntax error in %s at %s",
	...
	for (Ox = 0; Ox < TotalOwners; Ox++) {
	  ...
	  if (vlist.size() < 3) {
	    if (outmessage != NULL) {
	      sprintf(messageBuffer,"Syntax error in owners file (%s) at %s",
		      ownersFile.FullPath().c_str(),line.c_str());
	      *outmessage = new char[strlen(messageBuffer)+1];
	      strcpy(*outmessage,messageBuffer);
	    }
