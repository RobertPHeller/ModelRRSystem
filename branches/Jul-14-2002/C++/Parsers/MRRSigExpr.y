...
%define MEMBERS virtual ~MRRSigExpr() {}\
		...
		char *lp;\
		TreeTable *trees;\
		Table *FINDTABLE(const char * name) {return trees->lookuptable(name);}\
		Turnout *FINDTURNOUT(const char * name) {return trees->lookupturnout(name);}\
		Block *FINDBLOCK(const char * name) {return trees->lookupblock(name);}
