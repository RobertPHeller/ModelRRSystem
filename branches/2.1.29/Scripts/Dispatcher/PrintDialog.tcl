...
namespace eval PrintDialog {
  snit::widgetadaptor PrintDialog {
    ...
    constructor {args} {
      ...
      $hull add -name print -text [_m "Button|Print"] -command [mymethod _Print]
      ...
      $hull add -name help -text [_m "Button|Help"] -command {HTMLHelp::HTMLHelp help {Print Dialog}}
      set frame [$hull getframe]
      install printerRB using radiobutton $frame.printerRB \
				-command [mymethod _TogglePF] \
				-indicatoron yes \
				-value printer \
				-text [_ "Print to printer"] \
				-variable [myvar _FileOrPrinter]
      pack $printerRB -fill x
      set lwidth [_mx "Label|Printer:" "Label|File:" ]
      install printerCB using LabelComboBox $frame.printerCB -label [_m "Label|Printer:"] \
						    -labelwidth $lwidth \
						    -editable no
      ...
      install fileRB using radiobutton $frame.fileRB \
				...
				-indicatoron yes \
				-value file \
				-text [_m "Button|Print to file"] \
				-variable [myvar _FileOrPrinter]
      pack $fileRB -fill x
      install pfileFE using FileEntry $frame.pfileFE -label [_m "Label|File:"] -state disabled \
					    -labelwidth $lwidth \
					    -filedialog save \
					    -defaultextension .ps \
					    -filetypes $_PostscriptFiles \
					    -title [_ "File to print to"]
      pack $pfileFE -fill x
	...
	printer {
	  if {[string length "$lpr"] > 0} {
	    ...
	    error [_ "Cannot find print command!"]
	  }
	}
	file {
