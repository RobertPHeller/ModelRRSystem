...
proc Dialog::draw { path {focus ""} {overrideredirect 0} {geometry ""}} {
    ...
    puts stderr "*** Dialog::draw: \[winfo exists $parent\] = [winfo exists $parent]"
    puts stderr "*** Dialog::draw: \[wm state [winfo toplevel $parent]\] = [wm state [winfo toplevel $parent]]"
    puts stderr "*** Dialog::draw: \[winfo viewable $path\] (#1) = [winfo viewable $path]"
    if {![winfo exists $parent] || \
    ...
    puts stderr "*** Dialog::draw: \[winfo viewable $path\] (#2) = [winfo viewable $path]"
...
package provide FCFdialog 1.7.1
