...
catch {TimeTable::SplashWorkMessage {Loading Train Code} 44}
...
snit::type TimeTable::SelectOneTrainDialog {
  ...
  typemethod createDialog {} {
    ...
    $dialog add -name help -text Help -command [list HTMLHelp::HTMLHelp help {Select One Train Dialog}]
...
$TimeTable::Main menu add view command -label {View One Train} -command TimeTable::ViewOneTrain \
		      -dynamichelp "View a single train" -state disabled
$TimeTable::Main menu add view command -label {View All Trains} -command TimeTable::ViewAllTrains \
		      -dynamichelp "View all trains" -state disabled 
}
...
snit::type TimeTable::editTrainDialog {
  ...
  typemethod createDialog {} {
    ...
    $dialog add -name cancel -text Cancel -command [mytypemethod _Cancel]
    wm protocol [winfo toplevel $dialog] WM_DELETE_WINDOW [mytypemethod _Cancel]
    $dialog add -name help -text Help -command [list HTMLHelp::HTMLHelp help {Edit Train Dialog}]
    set frame [$dialog getframe] 
    set headerframe $frame.headerframe
    set iconimage $headerframe.iconimage
    set headerlabel $headerframe.headerlabel
...
$TimeTable::Main menu add trains command -label {Add Train} -command TimeTable::AddTrain \
			-dynamichelp "Add a new train"
#$TimeTable::Main menu add trains command -label {Edit Train} -command TimeTable::EditTrain \
#			-dynamichelp "Edit a train"
$TimeTable::Main menu add trains command -label {Delete Train} -command TimeTable::DeleteTrain \
			-dynamichelp "Delete a train"
$TimeTable::Main buttons add -name addTrain -anchor w \
	-text {Add a new train} -command TimeTable::AddTrain \
	-helptext "Add a new train" -state disabled
#$TimeTable::Main buttons add -name editTrain -anchor w \
#	-text {Edit an existing train} -command TimeTable::EditTrain \
#	-helptext "Edit an existin train" -state disabled
$TimeTable::Main buttons add -name deleteTrain -anchor w \
	-text {Delete an existing train} -command TimeTable::DeleteTrain \
	-helptext "Delete an existing train" -state disabled
...
#			-helptext "Edit an existing train" -state disabled
