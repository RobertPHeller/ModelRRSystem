 ...
 * kernels and GLIBC 2.2 (RedHat 7.3), and should work just fine with
 * 2.6 kernels. And it can use any serial port device supported by these
 * kernels.  That is, in addition to the standard four COM ports, it can
 * also use the various supported multi-port cards as well.
 * 
...
%typemap(in) CardType {
	...
	if (p == NULL || strlen(p) < 1) {
		Tcl_SetStringObj(tcl_result,_("Missing CardType, should be one of N, X, M, USIC, SUSIC, or SMINI!"),-1);
		return TCL_ERROR;
	}
	switch (toupper(p[0])) {
		case 'N': $1 = USIC; break;
		...
		default:
			if (strncasecmp(p,"USIC",strlen(p)) == 0) $1 = USIC;
				...
				Tcl_SetStringObj(tcl_result,_("Bad CardType, should be one of N, X, M, USIC, SUSIC, or SMINI! Got: "),-1);
